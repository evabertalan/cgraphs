...
from .proteingraphanalyser import (
    ProteinGraphAnalyser,
)  # TODO: make independnet form this module
...
class WaterClusters(ProteinGraphAnalyser):
    def __init__(
        ...
        ProteinGraphAnalyser.__init__(
            self,
            pdb_root_folder=pdb_root_folder,
        ...
        if (
            waters <= len(self.file_list) * 2
        ):  # check this number or fiugre out somethinf
            self.logger.warning(
                "There are not enough waters to cluster in the PDB files. Cluster analysis can not be performed."
            )
            return
        else:
            self.water_cluster_folder = _hf.create_directory(
                self.workfolder + "/water_clusters/"
            )
            ...
            if (
                len(self.superimposed_files) == 1
            ):  # check this number or fiugre out somethinf
                self.logger.warning(
                    "There is only 1 superimposed PDB structure. Cluster analysis can not be performed."
                )
                return
            else:
                ...
                self.logger.info(
                    "There are "
                    + str(len(self.water_coordinates))
                    + " water molecules in the "
                    + str(len(self.superimposed_files))
                    + " superimposed files"
                )
    ...
    def fit_parameters(self):
        ...
        self.parameter_folder = _hf.create_directory(
            self.water_cluster_folder + "/parameter_analysis/"
        )

        self.logger.info("Calculating nearest neighbor distances of data points.")
        ...
        self.logger.debug(
            "Calculating second derivative of nearest neighbor distances."
        )
        fig, ax = _hf.create_plot(
            figsize=(12, 8),
        ...
        indexes = [
            i for i in range(len(second_deriv)) if 0.02 <= second_deriv[i] <= 0.10
        ]
        ...
        for _eps in eps_candidates:
            ...
            if n_clusters_ < 2:
                self.logger.warning(
                    f"With eps: {np.round(_eps, 2)}  only 1 water cluster was found. Result of water clustering might be unreliable."
                )
                silhouette_score.append(-1)
            else:
                silhouette_score.append(
                    metrics.silhouette_score(self.water_coordinates, labels)
                )

        self.logger.info("Saving evaluation of possible eps values.")
        np.savetxt(
            self.parameter_folder + "eps_evaluation.csv",
            np.c_[eps_candidates, eps_cluster, eps_noise, silhouette_score],
            delimiter=",",
            fmt="%.2f",
            header="eps_candidate, number_of_cluster, number_of_outliers, silhouette_score",
        )

        self.logger.info(
            "Best eps is where the number of clusters are the maximal with low number of outliers and the silhouette score is maximal."
        )
        # max_sc = np.argmax(silhouette_score)
        if np.max(silhouette_score) < 0:
            self.logger.warning(
                "Silhouette score is negative. Result of water clutering might be unreliable."
            )
            self.logger.info(
                "Water molecules in the selected structre set do not show reliable clustering pattern."
            )
        else:
            indexes = [
                i
                for i in range(len(silhouette_score))
                if silhouette_score[i] >= np.max(silhouette_score) * 0.98
            ]
        self.logger.info(
            "Maximum silhouette score is: " + str(np.max(silhouette_score))
        )
        ...
        self.logger.info(
            "The best eps is: "
            + str(np.round(_eps[best_eps_index], 2))
            + ". With "
            + str(max_clusters[best_eps_index])
            + " clusters and "
            + str(min_outlier[best_eps_index])
            + " outliers."
        )
    ...
    def evaluate_parameters(self, eps=1.4, min_samples=None):
        ...
        self.logger.info(
            "Minimum number of water molecules to be considered as a cluster: "
            + str(min_samples)
        )
        if eps < 1.0 or eps > 1.6:
            self.logger.warning(
                "For water cluster analysis the recommended eps is 1.4. See parameter study in supporting information of: https://www.sciencedirect.com/science/article/pii/S1047847720302070"
            )

        db = DBSCAN(eps=eps, min_samples=min_samples).fit(
            self.water_coordinates[:, 0:3]
        )
        core_samples_mask = np.zeros_like(db.labels_, dtype=bool)
        core_samples_mask[db.core_sample_indices_] = True
        ...
        if self.n_clusters_ == 0:
            self.logger.warning(
                "No water cluster was found in the analyzed structures!"
            )
            return
        ...
        fig, ax = _hf.create_plot(
            title=f"Projection of the {self.n_clusters_} water clusters and {n_noise_} outlier points",
        ...
        for form in self.plot_parameters["formats"]:
            plt.savefig(
                f"{self.water_cluster_folder}water_clusters.{form}",
                format=form,
                dpi=self.plot_parameters["plot_resolution"],
            )
        self.logger.debug(
            "Water cluster plot is saved to :"
            + self.water_cluster_folder
            + "water_clusters.png"
        )
        plt.close()

    def calculate_cluster_centers(self):
        # append water center coordinates to reference coordinates with water cluster number
        self.clusters = {}
    ...
    def write_cluster_center_coordinates(self):
        np.savetxt(
            self.water_cluster_folder + "DBSCAN_water_cc_coordinates.txt",
            np.array(list(self.cluster_centers.values())),
        )
        with open(
            self.water_cluster_folder + "DBSCAN_water_cc_coordinates.xyz", "a"
        ) as file:
    ...
    def plot_all_waters(self, file_name=""):
        ...
        fig, ax = _hf.create_plot(
            title="Projection of all water molecules",
            xlabel="PCA projected xy plane",
            ylabel="Z coordinates",
            plot_parameters=self.plot_parameters,
        )
    ...
    def _get_water_coordinates(self):
        ...
        for file in self.superimposed_files:
            water_coord = _hf.water_coordinates(
                self.superimposed_structures_folder + file
            )
            ...
            np.savetxt(
                self.water_cluster_folder + _file_name + "_water_coordinates.txt",
                water_coord,
            )
