...
def plot_settings(self):
    ...
    settings_options = {
                ...
                'default': '20'},
                'plot_label_fontsize': {'label': 'Plot axis label font size',
                'default': '36'},
    ...
    tk.Label(main_frame, text='Graph color', anchor='w',  bg='white', fg='black').grid(row=i+1, column=0, sticky='W', pady=(2,2), padx=(2,2))
    ...
    tk.Label(main_frame, text='Water node color', anchor='w',  bg='white', fg='black').grid(row=i+2, column=0, sticky='W', pady=(2,2), padx=(2,2))
    self.water_node_color = '#db5c5c'
    water_node_color_field = tk.Label(main_frame, width=2, bg=self.water_node_color, anchor="w")
    water_node_color_field.grid(row=i+2, column=1, sticky='W', pady=(2,2), padx=(2,2))
    ...
    tk.Label(main_frame, text='Difference graph color', anchor='w',  bg='white', fg='black').grid(row=i+3, column=0, sticky='W', pady=(2,2), padx=(2,2))
    ...
    difference_graph_color_field.grid(row=i+3, column=1, sticky='W', pady=(2,2), padx=(2,2))
    ...
    self.png.set(True)
    tk.Checkbutton(main_frame, text='png', state='disabled', variable=self.png, anchor="w", bg='white', fg='black').grid(row=i+4, column=1, sticky='W')

    self.eps = tk.BooleanVar()
    self.eps.set(False)
    tk.Checkbutton(main_frame, text='eps', variable=self.eps, anchor="w", bg='white', fg='black').grid(row=i+5, column=1, sticky='W')

    self.svg = tk.BooleanVar()
    self.svg.set(False)
    tk.Checkbutton(main_frame,text='svg', variable=self.svg, anchor="w", bg='white', fg='black').grid(row=i+6, column=1, sticky='W')

    tk.Button(main_frame, text='Save', bg='white', fg='black', command=self._save_plot_settings, takefocus=False,  highlightbackground='white').grid(row=i+7, column=0, sticky="EW", columnspan=2)
