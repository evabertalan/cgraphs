...
def csa_view(self):
    ...
    tk.Button(
        self.inputFrame,
        text="Select PDB folder",
        command=lambda: self._select_pdb_root_folder(self._input_folder),
        takefocus=False,
        bg="white",
        fg="black",
        highlightbackground="white",
    ...
    tk.Button(
        ...
        command=lambda: self._select_reference_pdb(self._input_pdb),
        highlightbackground="white",
        bg="white",
        fg="black",
    ).grid(row=4, column=0, sticky="EW")
    s2 = self._add_horisontal_scroll(self.inputFrame, row=5, column=1)
    self._input_pdb = tk.Entry(
        self.inputFrame,
        state="disabled",
    ...
    tk.Label(
        self.inputFrame, text="Minimum sequence identity (%)", bg="white", fg="black"
    ).grid(row=6, column=0, sticky="W")
    ttk.Spinbox(
        self.inputFrame,
        textvariable=self.sequance_identity_threshold,
        from_=1,
        to=100,
        validate="key",
    ...
    ttk.Spinbox(
        self.inputFrame,
        textvariable=self.conservation_threshold,
        from_=1,
        to=100,
        width=5,
        validate="key",
    ...
    self.waterClusterFrame = ttk.LabelFrame(
        self.mainframe, text="Water cluster analysis with DBSCAN"
    )
    ...
    lwcp = tk.Label(
        waterClusterParameterFrame,
        text="For the DBSCAN analysis the default parameters are recommended to use.",
        anchor="w",
        bg="white",
        fg="black",
    )
    ...
    tk.Label(
        waterClusterParameterFrame, text="DBSCAN eps:", bg="white", fg="black"
    ...
    tk.Label(
        waterClusterParameterFrame,
        text="Superimposition RMS threshold:",
        bg="white",
        fg="black",
    ...
    ttk.Spinbox(
        waterClusterParameterFrame,
        textvariable=self.superimposition_threshold,
        width=5,
        validate="key",
        validatecommand=(self.ifnum_cmd, "%S", "%P", 0, 100),
    ...
    tk.Button(
        ...
        text="Calculate water clusters",
        command=self._init_water_clusters,
        width=self.button_width,
        bg="white",
        fg="black",
        highlightbackground="white",
    ).grid(self._create_big_button_grid(2))

    # ----------------------- conservedNetworkFrame -----------------------

    self.conservedNetworkFrame = ttk.LabelFrame(
        self.mainframe, text="Conserved network analysis with Bridge"
    )
    ...
    self.selection_string, self.selected_donors_pdb, self.selected_acceptors_pdb = (
        self.custom_selection_string(selsting_frame, 8)
    )

    tk.Label(
        self.conservedNetworkFrame,
        text="H-bond criteria ",
        anchor="w",
        bg="white",
        fg="black",
    ...
    tk.Label(
        hcritera_frame, text="Å distance     ", anchor="w", bg="white", fg="black"
    ).grid(row=9, column=2, sticky="W")
    self.c_use_angle = tk.BooleanVar()
    self.c_use_angle.set(False)
    tk.Checkbutton(
        hcritera_frame, variable=self.c_use_angle, anchor="w", bg="white", fg="black"
    ).grid(row=9, column=3, sticky="E")
    ttk.Spinbox(
        hcritera_frame,
        textvariable=self.c_cut_angle,
        from_=0,
        to=180,
        width=5,
        validate="key",
    ...
    tk.Label(
        hcritera_frame, text="degrees angle", anchor="w", bg="white", fg="black"
    ...
    tk.Button(
        ...
        command=lambda: self.node_color_selelection_pop_up(
            self.selected_nodes_for_color, self.selected_color_map
        ),
        bg="white",
        fg="black",
        highlightbackground="white",
    ...
    tk.Checkbutton(
        ...
        variable=self.is_induvidual_graph,
        anchor="w",
        bg="white",
        fg="black",
    ...
    tk.Checkbutton(
        ...
        variable=self.is_difference_graph,
        anchor="w",
        bg="white",
        fg="black",
    ...
    tk.Checkbutton(
        each_plots_crystal,
        text="Linear lengths",
        variable=self.is_linear_lenght_plot,
        anchor="w",
        bg="white",
        fg="black",
    ...
    tk.Checkbutton(
        each_plots_crystal,
        text="H-bond distances",
        variable=self.is_bond_distance_plot,
        anchor="w",
        bg="white",
        fg="black",
    ...
    self.HbondNetworkFrame = ttk.LabelFrame(
        self.conservedNetworkFrame, text="H-bond network"
    )
    ...
    tk.Checkbutton(
        self.HbondNetworkFrame,
        text="Include crystallographic waters",
        variable=self.include_waters_hbond,
        anchor="w",
        bg="white",
        fg="black",
    ).grid(self._create_big_button_grid(14))

    self.useWaterCoords = tk.BooleanVar()
    tk.Checkbutton(
        self.HbondNetworkFrame,
        text="Use water cluster coordinates",
        variable=self.useWaterCoords,
        anchor="w",
        bg="white",
        fg="black",
    ).grid(self._create_big_button_grid(15))

    self.include_backbone_sidechain = tk.BooleanVar()
    tk.Checkbutton(
        self.HbondNetworkFrame,
        text="Include sidechain-backbone interactions",
        variable=self.include_backbone_sidechain,
        anchor="w",
        bg="white",
        fg="black",
    ).grid(self._create_big_button_grid(16))

    tk.Button(
        self.HbondNetworkFrame,
        text="Calculate conserved H-bond network",
        command=lambda: self._init_pdb_conserved_graph_analysis("hbond"),
        width=self.button_width,
        bg="white",
        fg="black",
        highlightbackground="white",
    ).grid(self._create_big_button_grid(17))

    # ----------------------- WaterWireFrame -----------------------

    self.WaterWireFrame = ttk.LabelFrame(
        self.conservedNetworkFrame, text="Water wire network"
    )
    ...
    ttk.Combobox(
        self.WaterWireFrame,
        textvariable=self.max_water,
        values=[1, 2, 3, 4, 5],
        state="readonly",
    ).grid(row=17, column=1, sticky="EW")
    tk.Button(
        self.WaterWireFrame,
        text="Calculate conserved water wire network",
        command=lambda: self._init_pdb_conserved_graph_analysis("water_wire"),
        width=self.button_width,
        bg="white",
        fg="black",
        highlightbackground="white",
    ).grid(self._create_big_button_grid(18), columnspan=2)
