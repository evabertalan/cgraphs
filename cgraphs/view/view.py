...
class popupWindow(object):
    ...
    def custom_selection_sting(
        self, selection_entry, selected_donors, selected_acceptors
    ):
        tk.Label(
            self.top,
            text="Customize selection string and additional donors and acceptors",
            bg="white",
            fg="black",
            pady=4,
        ).grid(row=0, column=0, sticky="EW", columnspan=3)
        tk.Label(self.top, text="Selection string:", bg="white", fg="black").grid(
            row=1, column=0, sticky="W"
        )
        scroll = tk.Scrollbar(self.top, orient="horizontal")
        self.sel_string = tk.Entry(
            self.top,
        ...
        tk.Label(
            self.top, text="List of additional donors:", bg="white", fg="black"
        ).grid(row=4, column=0, sticky="W")
        self.sel_donors = tk.Entry(
            self.top,
            bg="white",
            fg="black",
            highlightbackground="white",
        ...
        tk.Label(
            self.top, text="List of additional acceptors:", bg="white", fg="black"
        ).grid(row=5, column=0, sticky="W")
        self.sel_acceptors = tk.Entry(
            self.top,
            bg="white",
            fg="black",
            highlightbackground="white",
    ...
    def node_color_selection(self, selected_nodes_for_color, selected_color):
        tk.Label(
            self.top,
            text="Specify residues for node coloring",
            bg="white",
            fg="black",
            pady=4,
        ).grid(row=0, column=0, sticky="EW", columnspan=3)
        ...
        tk.Label(self.top, text="Color map:", bg="white", fg="black").grid(
            row=2, column=0, sticky="W"
        )
        tk.OptionMenu(self.top, self.nc_color_map, *color_options).grid(
...
class View:
    ...
    def _select_reference_pdb(self, field):
        self.reference_pdb = filedialog.askopenfilename(
            filetypes=[("pdb", ".pdb")], parent=self.mainframe
        )
    ...
    def _init_water_clusters(self):
        if not hasattr(self, "pdb_root_folder") or self.pdb_root_folder is None:
            ...
            if self.w.valid_structures_for_clustering:
                ...
                tk.Label(
                    self.waterClusterFrame,
                    text=" There are "
                    + str(len(self.w.water_coordinates))
                    + " water molecules in the "
                    + str(len(self.w.superimposed_files))
                    + " superimposed files.\n The algorithm found "
                    + str(self.w.n_clusters_)
                    + " water clusters.",
                    anchor="w",
                    justify=tk.LEFT,
                    fg="green",
                ...
                self.w.logger.info(
                    "Water cluster calculation is completed\n" + "-" * 20
                )
    ...
    def _init_pdb_conserved_graph_analysis(self, graph_type):
        if not hasattr(self, "pdb_root_folder") or self.pdb_root_folder is None:
            ...
            if self.useWaterCoords.get() and (
                not hasattr(self, "ref_coordinates") or self.ref_coordinates is None
            ):
                print("WARNING: There are no water cluster coordinates!")
            else:
                additional_donors = self._create_list_from_sting(
                    self.selected_donors_pdb.get()
                )
                additional_acceptors = self._create_list_from_sting(
                    self.selected_acceptors_pdb.get()
                )
                ...
                if graph_type == "water_wire":
                    c.calculate_graphs(
                        ...
                        max_water=int(self.max_water.get()),
                        distance=float(self.c_distance.get()),
                        cut_angle=float(self.c_cut_angle.get()),
                        check_angle=self.c_use_angle.get(),
                        selection=self.selection_string.get(),
                    ...
                    c.calculate_graphs(
                        graph_type=graph_type,
                        exclude_backbone_backbone=ebb,
                        include_backbone_sidechain=ieb,
                        include_waters=self.include_waters_hbond.get(),
                        distance=float(self.c_distance.get()),
                        cut_angle=float(self.c_cut_angle.get()),
                        check_angle=self.c_use_angle.get(),
                        selection=self.selection_string.get(),
                ...
                self._plot_conserved_graphs(
                    c,
                    self.is_linear_lenght_plot.get(),
                    self.is_induvidual_graph.get(),
                    self.is_difference_graph.get(),
                    self.is_bond_distance_plot.get(),
    ...
    def _select_psf_file(self):
        self.psf_file = filedialog.askopenfilename(
            title="Select protein structure file file",
            filetypes=[("psf", ".psf"), ("pdb", ".pdb"), ("gro", ".gro")],
            parent=self.DcdWaterWireFrame,
        )
        self._configure_entry_field(self._input_psf, self.psf_file)

    def _select_dcd_files(self):
        self.dcd_files = filedialog.askopenfilenames(
            title="Select trajectory files",
            filetypes=[("dcd", ".dcd"), ("xtc", ".xtc"), ("trr", ".trr")],
            parent=self.DcdWaterWireFrame,
        )
    ...
    def _construct_sim_graphs(self):
        if not hasattr(self, "_target_folder") or self._target_folder is None:
            ...
            if self.DcdInfoFrame:
                self.DcdInfoFrame.destroy()
            additional_donors = self._create_list_from_sting(
                self.sim_selected_donors_pdb.get()
            )
            additional_acceptors = self._create_list_from_sting(
                self.sim_selected_acceptors_pdb.get()
            )
            p = ProteinGraphAnalyser(
                type_option="dcd",
                dcd_files=[self.dcd_files],
                psf_files=[self.psf_file],
                sim_names=[self.sim_name.get()],
                target_folder=self._target_folder,
                plot_parameters=self.plot_parameters,
            )
            p.calculate_graphs(
                graph_type="water_wire",
                max_water=int(self.sim_max_water.get()),
                distance=float(self.sim_distance.get()),
                cut_angle=float(self.sim_cut_angle.get()),
                check_angle=self.sim_use_angle.get(),
                selection=self.sim_selection_string.get(),
                additional_donors=additional_donors,
                additional_acceptors=additional_acceptors,
            )
            ...
            tk.Label(
                self.DcdInfoFrame,
                text="Calculation completed for " + self.sim_name.get(),
                fg="green",
                anchor="w",
                bg="white",
    ...
    def _init_dcd_conserved_graph_analysis(self):
        if not hasattr(self, "_target_folder") or self._target_folder is None:
            ...
            tk.Label(
                self.DcdOptionsFrame,
                text="Conservation of H-bonding groups across structures (%)",
                anchor="w",
                bg="white",
                fg="black",
            ...
            ttk.Spinbox(
                self.DcdOptionsFrame,
                textvariable=self.conservation_threshold_dcd,
                from_=1,
                to=100,
                width=5,
                validate="key",
                validatecommand=(self.ifnum_cmd, "%S", "%P", 0, 100),
            ).grid(row=self.row + 2, column=1, sticky="EW")
            self.min_occupancy = tk.DoubleVar(value=10)
            tk.Label(
                self.DcdOptionsFrame,
                text="Minimum H-bond occupancy (%)",
                anchor="w",
                bg="white",
                fg="black",
            ).grid(row=self.row + 3, column=0, sticky="W")
            ttk.Spinbox(
                self.DcdOptionsFrame,
                textvariable=self.min_occupancy,
                from_=1,
                to=100,
                width=5,
                validate="key",
                validatecommand=(self.ifnum_cmd, "%S", "%P", 0, 100),
            ).grid(row=self.row + 3, column=1, sticky="EW")

            tk.Label(
                self.DcdOptionsFrame,
                text="Plot for each structure:",
                anchor="w",
                bg="white",
                fg="black",
            ).grid(row=self.row + 4, column=0, sticky="W")
            each_plots_dcd = tk.Frame(self.DcdOptionsFrame, bg="white")
            each_plots_dcd.grid(row=self.row + 4, column=1, columnspan=3, sticky="EW")

            tk.Checkbutton(
                each_plots_dcd,
                text="Individual network    ",
                variable=self.is_induvidual_graph_dcd,
                anchor="w",
                bg="white",
                fg="black",
            ).grid(row=self.row + 4, column=1)
            tk.Checkbutton(
                each_plots_dcd,
                text="Difference graph    ",
                variable=self.is_difference_graph_dcd,
                anchor="w",
                bg="white",
                fg="black",
            ).grid(row=self.row + 4, column=2)
            tk.Checkbutton(
                each_plots_dcd,
                text="Linear lengths",
                variable=self.is_linear_lenght_plot_dcd,
                anchor="w",
                bg="white",
                fg="black",
            ).grid(row=self.row + 4, column=3)

            self.color_dcd_edge_data = tk.BooleanVar()
            self.color_dcd_edge_data.set(False)
            tk.Checkbutton(
                self.DcdOptionsFrame,
                text="Color edges by user defined values",
                variable=self.color_dcd_edge_data,
                anchor="w",
                bg="white",
                fg="black",
            ).grid(row=self.row + 5, column=0, sticky="W")
            sl = tk.Label(
                self.DcdOptionsFrame,
                text='To color edges, place a file named "<any name>_color_edges.txt" in the folder of the .psf file.\nAbout the format, please find more information in the documentation.',
                anchor="w",
                justify="left",
                bg="white",
                fg="black",
            ...
            self.dcd_calc_button = tk.Button(
                self.LoadGraphFrame,
                text="Calculate conserved network",
                command=lambda: self._plot_conserved_graphs(
                    c_dcd,
                    self.is_linear_lenght_plot_dcd.get(),
                    self.is_induvidual_graph_dcd.get(),
                    self.is_difference_graph_dcd.get(),
                    False,
                    cth=int(self.conservation_threshold_dcd.get()) / 100,
                    occupancy=int(self.min_occupancy.get()) / 100,
                    color_edges=self.color_dcd_edge_data.get(),
                ),
    ...
    def _load_graph_files(self, row):
        if not hasattr(self, "_target_folder") or self._target_folder is None:
            ...
            if self.dcd_load_button:
                ...
                self.graph_files = None
            self.graph_files = filedialog.askopenfilenames(
                initialdir=self._target_folder + "/workfolder/graph_objects/",
                title="Select simulation graphs",
            ...
            if self.graph_files:
                ...
                self.LoadGraphFrame.grid(
                    self._crate_frame_grid(row, columnspan=2), sticky="EW"
                )
                self.LoadGraphFrame.columnconfigure(0, weight=1)
                tk.Label(
                    self.LoadGraphFrame,
                    text="Selected simulations for conserved network calculation: ",
                    anchor="w",
                    bg="white",
                    fg="black",
                ...
                self.graph_text_field = tk.Text(
                    self.LoadGraphFrame,
                    height=4,
                    yscrollcommand=sh.set,
                    bg="white",
                    fg="black",
                    highlightbackground="white",
                )
                ...
                self.dcd_load_button = tk.Button(
                    self.LoadGraphFrame,
                    text="Load graphs",
                    command=self._init_dcd_conserved_graph_analysis,
                    width=self.button_width,
                    bg="white",
                    fg="black",
                    highlightbackground="white",
    ...
    def _select_pdb1(self, field):
        self.pdb_1 = filedialog.askopenfilename(
            filetypes=[("pdb", ".pdb")], parent=self.compframe
        )
    ...
    def _select_pdb2(self, field):
        self.pdb_2 = filedialog.askopenfilename(
            filetypes=[("pdb", ".pdb")], parent=self.compframe
        )
    ...
    def _select_psf1_compare(self, field):
        self.psf_1 = filedialog.askopenfilename(
            title="Select protein structure file file",
            filetypes=[("psf", ".psf"), ("pdb", ".pdb"), ("gro", ".gro")],
            parent=self.DcdWaterWireFrame,
        )
    ...
    def _select_dcd1_compare(self, field):
        self.dcd_1 = filedialog.askopenfilenames(
            title="Select trajectory files",
            filetypes=[("dcd", ".dcd"), ("xtc", ".xtc"), ("trr", ".trr")],
            parent=self.compframe,
        )
    ...
    def _select_psf2_compare(self, field):
        self.psf_2 = filedialog.askopenfilename(
            title="Select protein structure file file",
            filetypes=[("psf", ".psf"), ("pdb", ".pdb"), ("gro", ".gro")],
            parent=self.DcdWaterWireFrame,
        )
    ...
    def _select_dcd2_compare(self, field):
        self.dcd_2 = filedialog.askopenfilenames(
            title="Select trajectory files",
            filetypes=[("dcd", ".dcd"), ("xtc", ".xtc"), ("trr", ".trr")],
            parent=self.compframe,
        )
    ...
    def _init_pdb_comparison(
        self, comp_type, pdb1, pdb2, color1="#1b3ede", color2="#21c25f"
    ):
        if (
            not hasattr(self, "compare_results_folder")
            or self.compare_results_folder is None
        ):
            print("WARNING: Please select the location of the workfolder!")
        else:
            comp = CompareTwo(
            ...
            additional_acceptors = self._create_list_from_sting(
                self.pdb_comp_selected_acceptors_pdb.get()
            )
            comp.calculate_graphs(
                graph_type=comp_type,
                max_water=self.max_water_comp.get(),
                include_backbone_sidechain=self.include_backbone_sidechain_comp.get(),
                include_waters=self.include_waters_comp.get(),
                distance=self.comp_distance.get(),
                cut_angle=self.comp_cut_angle.get(),
                check_angle=self.comp_use_angle.get(),
                selection=self.pdb_comp_selection_string.get(),
                additional_donors=additional_donors,
                additional_acceptors=additional_acceptors,
            )
    ...
    def _construct_compare_graphs(
        ...
        if (
            ...
            self.comp = CompareTwo(
                ...
                dcd2=dcd2,
                target_folder=self.compare_results_folder,
                name1=self.compare_dcd1_name.get(),
                name2=self.compare_dcd2_name.get(),
                plot_parameters=self.plot_parameters,
            )
            additional_donors = self._create_list_from_sting(
                self.dcd_comp_selected_donors_pdb.get()
            )
            additional_acceptors = self._create_list_from_sting(
                self.dcd_comp_selected_acceptors_pdb.get()
            )
            self.comp.calculate_graphs(
                graph_type="water_wire",
                max_water=self.max_water_comp_dcd.get(),
                distance=self.comp_distance.get(),
                cut_angle=self.comp_cut_angle.get(),
                check_angle=self.comp_use_angle.get(),
                selection=self.dcd_comp_selection_string.get(),
                additional_donors=additional_donors,
                additional_acceptors=additional_acceptors,
            )

            # -------------------water_wire_frame -----------------------
            if self.water_wire_comp_frame_dcd:
                self.water_wire_comp_frame_dcd.destroy()
            self.water_wire_comp_frame_dcd = ttk.LabelFrame(
                self.dcd_compare_tab, text="Water wire network"
            )
            self.water_wire_comp_frame_dcd.grid(
                self._crate_frame_grid(self.compare_row)
            )
            ...
            tk.Label(
                self.water_wire_comp_frame_dcd,
                text="Minimum H-bond occupancy (%)",
                anchor="w",
                bg="white",
                fg="black",
            ...
            ttk.Spinbox(
                self.water_wire_comp_frame_dcd,
                textvariable=self.min_occupancy_comp,
                from_=1,
                to=100,
                validate="key",
                validatecommand=(self.ifnum_cmd, "%S", "%P", 0, 100),
            ...
            tk.Button(
                self.water_wire_comp_frame_dcd,
                text="Compare water wire network",
                command=lambda: self._plot_dcd_comparison(
                    color1=self.color_dcd1, color2=self.color_dcd2
                ),
                width=self.button_width,
                bg="white",
                fg="black",
                highlightbackground="white",
            ).grid(self._create_big_button_grid(2), columnspan=2)

    def _plot_dcd_comparison(self, color1="#1b3ede", color2="#21c25f"):
        self.comp.construct_comparison_objects(
    ...
    def _plot_conserved_graphs(
        self,
        c,
        plot_linear_length,
        plot_induvidual_graph,
    ...
    def custom_selection_string(self, parent_frame, row):
        ...
        selection_entry = tk.Entry(
            parent_frame,
            state="disabled",
            bg="white",
            fg="black",
            highlightbackground="white",
            disabledbackground=self.gray,
        ...
        self.custom_selection_button = tk.Button(
            parent_frame,
            text="Selection",
            command=lambda: self.selection_string_popup(
                selection_entry, selected_donors, selected_acceptors
            ),
            takefocus=False,
            bg="white",
            fg="black",
    ...
    def _create_big_button_grid(self, row, column=0):
        return {"row": row, "column": column, "pady": (5, 5), "sticky": "EW"}
    ...
    def _create_frame(self):
        ...
        style.theme_create(
            ...
            settings={
                ...
                "TNotebook": {
                    "configure": {"tabmargins": [2, 5, 0, 0], "background": "white"}
                },
                "TFrame": {
                    "configure": {
                        "relief": "flat",
                ...
                "TLabelframe.Label": {
                    "configure": {"font": ("Helvetica", 13, "bold")},
                    "background": "white",
                },
                "TSpinbox": {
                    "configure": {
        ...
        scrollbar = tk.Scrollbar(
            self.master, orient="vertical", command=canvas.yview, bg="white"
        )
    ...
    def VaidateNum(self, S, P, _min, _max):
        ...
        valid = (
            (S.isdigit() or S == ".")
            and P != ""
            and float(P) >= float(_min)
            and float(P) <= float(_max)
        )
        if not valid:
