...
def ta_view(self):
    ...
    inital_sim_settings = ttk.LabelFrame(
        self.dcdframe, text="Parameter set up for the analysis"
    )
    ...
    tk.Button(
        ...
        bg="white",
        fg="black",
        command=lambda: self._select_dcd_workfolder(self._input_target),
        takefocus=False,
        highlightbackground="white",
    ).grid(
        row=0,
    ...
    ttk.Combobox(
        inital_sim_settings,
        textvariable=self.sim_max_water,
        values=[1, 2, 3, 4, 5],
        state="readonly",
    ).grid(row=2, column=1, sticky="EW")

    tk.Label(
        inital_sim_settings, text="H-bond criteria ", anchor="w", bg="white", fg="black"
    ...
    ttk.Spinbox(
        sim_critera_frame,
        textvariable=self.sim_distance,
        from_=0,
        to=5,
        width=5,
        validate="key",
    ...
    tk.Label(
        sim_critera_frame, text="Å distance     ", anchor="w", bg="white", fg="black"
    ).grid(row=3, column=2, sticky="W")
    self.sim_use_angle = tk.BooleanVar()
    self.sim_use_angle.set(True)
    tk.Checkbutton(
        sim_critera_frame,
        variable=self.sim_use_angle,
        anchor="w",
        bg="white",
        fg="black",
    ...
    ttk.Spinbox(
        sim_critera_frame,
        textvariable=self.sim_cut_angle,
        from_=0,
        to=180,
        width=5,
        validate="key",
    ...
    tk.Label(
        sim_critera_frame, text="degrees angle", anchor="w", bg="white", fg="black"
    ...
    (
        self.sim_selection_string,
        self.sim_selected_donors_pdb,
        self.sim_selected_acceptors_pdb,
    ) = self.custom_selection_string(selsting_frame, 2)
    # --------------------------- dcd select------------
    ...
    sl = tk.Label(
        self.selectSimFrame,
        text="Compute H-bond graph for one simulation at a time. Once the graph is computed, the water wire for the current simulation may be calculated.\nAlternatively, the graph for another simulation may be computed.",
        anchor="w",
        justify="left",
        bg="white",
        fg="black",
    ...
    tk.Label(
        self.selectSimFrame, text="Name as: ", anchor="w", bg="white", fg="black"
    ).grid(row=9, column=0, sticky="W")
    # self.sim_name1 = tk.StringVar(value='sim1')
    self.sim_name = tk.Entry(
        self.selectSimFrame,
        bg="white",
        fg="black",
        highlightbackground="white",
    ...
    l1 = tk.Label(
        self.selectSimFrame,
        text="Each simulation must be given a unique name, else C-Graphs results will be overwritten.",
        anchor="w",
        bg="white",
        fg="black",
    )
    ...
    tk.Button(
        ...
        command=self._construct_sim_graphs,
        bg="white",
        fg="black",
        highlightbackground="white",
    ).grid(self._create_big_button_grid(11, column=1))
    ...
    self.row = 16
    tk.Button(
        self.DcdWaterWireFrame,
        text="Select graphs to compare",
        command=lambda: self._load_graph_files(self.row),
        bg="white",
        fg="black",
        highlightbackground="white",
