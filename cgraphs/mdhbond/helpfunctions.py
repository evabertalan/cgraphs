...
water_definition = "(resname TIP3 and name OH2) or (resname HOH and name O) or (resname TIP4 and name OH2)"
...
class Selection:
    def __init__(
        self, mda_residue, donor_names, acceptor_names, add_donors_without_hydrogen
    ):
        ...
        if mda_residue:
            for atom in mda_residue.atoms:
                if (atom.name in donor_names) or (
                    (len(atom.name) > 1) and atom.name[0] in ["N", "O"]
                ):
                    if add_donors_without_hydrogen:
                        ...
                        for otheratom in atom.residue.atoms:
                            if (
                                otheratom.name[0] == "H"
                                or otheratom.name[:2] in ["1H", "2H", "3H"]
                                or otheratom.type == "H"
                            ) and np.sqrt(
                                ((atom.position - otheratom.position) ** 2).sum()
                            ) < r_covalent[
                                atom.name[0]
                            ]:
                                ...
                                c += 1
                        if c > 0:
                            self.donor2hydrogens.append(
                                range(len(self.hydrogens) - c, len(self.hydrogens))
                            )
                            self.donors.append(atom)
                            self.donor_info.append(
                                str(atom.segid)
                ...
                if (atom.name in acceptor_names) or (
                    (len(atom.name) > 1) and atom.name[0] in ["N", "O"]
                ):
            ...
            if self.donors:
                self.donors = MDAnalysis.core.groups.AtomGroup(self.donors)
            if self.acceptors:
                self.acceptors = MDAnalysis.core.groups.AtomGroup(self.acceptors)
            if self.hydrogens:
                self.hydrogens = MDAnalysis.core.groups.AtomGroup(self.hydrogens)
            self.count = len(self.donors) + len(self.acceptors)
            if not self.count > 0:
                raise AssertionError("neither donors nor acceptors in the selection")
...
def MDA_info_list(group, detailed_info=False, special_naming=[]):
    ...
    for atom in group:
        ...
        if rname in special_naming:
            rname = str(atom.name)
        if detailed_info:
            result.append(sid + "-" + rname + "-" + rid + "-" + atom.name)
        else:
            result.append(sid + "-" + rname + "-" + rid)
    return result
...
def intervals_binary(timeseries):
    ...
    if ts[0]:
        if ts[-1]:
            changes = np.concatenate(([0], np.nonzero(np.diff(ts))[0] + 1, [ts.size]))
        else:
            changes = np.concatenate(([0], np.nonzero(np.diff(ts))[0] + 1))
    else:
        if ts[-1]:
            changes = np.concatenate((np.nonzero(np.diff(ts))[0] + 1, [ts.size]))
        else:
            changes = np.nonzero(np.diff(ts))[0] + 1
...
def block_analysis(dictionary, step_size, block_size, conv_cut):
    min_interval = range(
        0,
        int((len(dictionary[dictionary.keys()[0]]) - block_size) / step_size),
        step_size,
    )
    ...
    result = dict(
        zip(
            np.array(dictionary.keys())[conv_index],
            np.array(dictionary.values())[conv_index],
        )
    )
    return result
...
def filter_occupancy(dictionary, min_occupancy):
    for key in dictionary:
        ...
        break
    if dtype == np.int:
        values = np.array(np.array(list(dictionary.values())) < np.inf, dtype=np.float)
    else:
        values = np.array(list(dictionary.values()), dtype=np.float)
...
def predecessor_recursive(d, pred, start, stop):
    if d == 0:
        return pred[start, stop]
    else:
        return pred[start, predecessor_recursive(d - 1, pred, start, stop)]
...
def predecessor_recursive_1d(d, pred, start):
    if d == 0:
        ...
        return pred[predecessor_recursive_1d(d - 1, pred, start)]
...
def complete_subgraphs(edges_matrix):
    ...
    subgraphs = subgraphs[
        np.logical_not(
            [((subgraphs * p) == p).all(axis=1).sum() > 1 for p in subgraphs]
        )
    ]
    return subgraphs
