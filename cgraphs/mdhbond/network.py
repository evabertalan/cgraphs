...
class NetworkAnalysis(BasicFunctionality):
    ...
    def __init__(
        ...
        if restore_filename != None:
            return
        self.donor_names = _hf.donor_names_global.union(additional_donors) - set(
            exclude_donors
        )
        self.acceptor_names = _hf.acceptor_names_global.union(
            additional_acceptors
        ) - set(exclude_acceptors)
        self.check_angle = check_angle
        self.distance = distance
        self.cut_angle = cut_angle
        self._add_donors_without_hydrogen = add_donors_without_hydrogen
        sorted_selection = _hf.Selection(
            self._mda_selection,
            self.donor_names,
            self.acceptor_names,
            add_donors_without_hydrogen,
        )
        if not sorted_selection.donors:
            da_selection = sorted_selection.acceptors
        elif not sorted_selection.acceptors:
            da_selection = sorted_selection.donors
        else:
            da_selection = _MDAnalysis.core.groups.AtomGroup(
                sorted_selection.donors + sorted_selection.acceptors
            )
        self._da_selection = da_selection
        if sorted_selection.donors:
            self._donors = _MDAnalysis.core.groups.AtomGroup(sorted_selection.donors)
        else:
            self._donors = _hf.EmptyGroup()
        self._nb_donors = len(self._donors)
        if sorted_selection.acceptors:
            self._acceptors = _MDAnalysis.core.groups.AtomGroup(
                sorted_selection.acceptors
            )
        else:
            self._acceptors = _hf.EmptyGroup()
        ...
        if not add_donors_without_hydrogen:
            try:
                water_hydrogen = _MDAnalysis.core.groups.AtomGroup(
                    self._water[0].residue.atoms[1:]
                )
        ...
        if not sorted_selection.hydrogens and not water_hydrogen:
            if check_angle:
                raise AssertionError(
                    "There are no possible hbond donors in the selection and no water. Since check_angle is True, hydrogen is needed for the calculations!"
                )
            else:
                hydrogen = _hf.EmptyGroup()
        elif not sorted_selection.hydrogens:
            hydrogen = _MDAnalysis.core.groups.AtomGroup(water_hydrogen)
        elif not water_hydrogen:
            hydrogen = sorted_selection.hydrogens
        else:
            hydrogen = sorted_selection.hydrogens + _MDAnalysis.core.groups.AtomGroup(
                water_hydrogen
            )
        self._hydrogen = hydrogen
        self.heavy2hydrogen = (
            sorted_selection.donor2hydrogens
            + [[] for i in sorted_selection.acceptors]
            + [
                [
                    self._first_water_hydrogen_id + i,
                    self._first_water_hydrogen_id + i + 1,
                ]
                for i in range(0, len(water_hydrogen), 2)
            ]
        )
    ...
    def filter_occupancy(self, min_occupancy, use_filtered=True):
        ...
        filtered_result = {
            key: results[key]
            for key in results
            if _np.mean(results[key]) > min_occupancy
        }
        if self.applied_filters["occupancy"] != None:
            self.applied_filters["occupancy"] = max(
                self.applied_filters["occupancy"], min_occupancy
            )
        else:
            self.applied_filters["occupancy"] = min_occupancy
    ...
    def filter_all_paths(
        ...
        try:
            if only_shortest:
                paths = _nx.all_shortest_paths(component, start, goal)
            else:
    ...
    def filter_single_path(self, *nodes, use_filtered=True):
        ...
        for resa, resb in _hf.pairwise(nodes):
            ...
            if edge in graph.edges():
                ...
                raise AssertionError(
                    "There is no connection between {} and {}".format(resa, resb)
                )
    ...
    def filter_between_segnames(self, segna, segnb=None, use_filtered=True):
        ...
        for key in results:
            ...
            if (
                (sa in segnames) and (sb in segnames) and (sa != sb) and (segnb != None)
            ) or (
                _np.logical_xor((sa in segnames), (sb in segnames)) and (segnb == None)
            ):
    ...
    def filter_between_resnames(self, resna, resnb=None, use_filtered=True):
        ...
        for key in results:
            ...
            if (
                (ra in resnames) and (rb in resnames) and (ra != rb) and (resnb != None)
            ) or (
                _np.logical_xor((ra in resnames), (rb in resnames)) and (resnb == None)
            ):
                keep_bonds.append(key)
    ...
    def _lipid_analysis(
        ...
        correction_hba,
        between_nodes=None,
        filter_highest_occupancy=False,
        use_filtered=True,
    ):
        if use_filtered:
            graph = self.filtered_graph
        ...
        if between_nodes != None:
            nodes = between_nodes
        else:
            nodes = [key for key in correction_dict]
        res_length = {}
        res_path = {}
        res_occ = {}
        ...
        for keya, keyb in combinations(nodes, 2):
            ...
            try:
                ...
                for frame in range(self.nb_frames):
                    if not correction_dict[keya][frame]:
                        continue
                    if not correction_dict[keyb][frame]:
                        continue
                    frame_graph = self._compute_graph_in_frame(
                        frame, use_filtered=use_filtered
                    )
                    try:
                ...
                if filter_highest_occupancy:
                    ...
                    for path in temp_paths:
                        combined_occupancy.append(
                            _np.ones(self.nb_frames, dtype=_np.bool)
                        )
                        for nodea, nodeb in _hf.pairwise(path):
                            try:
                                combined_occupancy[-1] &= results[
                                    ":".join((nodea, nodeb))
                                ]
                            except KeyError:
                                combined_occupancy[-1] &= results[
                    ...
                    res_path[key] = [
                        int(node.split("-")[2]) for node in temp_paths[max_index]
                    ]
                    ...
                    res_occ[key] = occ
                    res_path[key] = [
                        [int(node.split("-")[2]) for node in path]
                        for path in temp_paths
                    ]
    ...
    def compute_all_pairs_shortest_path_per_frame(
        self,
        max_length=5,
        between_nodes=None,
        filter_highest_occupancy=False,
    ...
    def compute_all_pairs_shortest_path(
        self, max_length=5, between_nodes=None, use_filtered=True
    ):
        if use_filtered:
            graph = self.filtered_graph
            results = self.filtered_results
        ...
        for keya, keyb in combinations(nodes, 2):
            try:
                ...
                for nodea, nodeb in _hf.pairwise(path):
                    try:
                        combined_occupancy &= results[":".join((nodea, nodeb))]
                    except KeyError:
                        combined_occupancy &= results[":".join((nodeb, nodea))]
    ...
    def compute_total_occupancy(self, use_filtered=True):
        ...
        for key in results:
            resa, resb = key.split(":")
            try:
                return_res[resa] |= results[key]
            except KeyError:
                return_res[resa] = results[key]
            try:
                return_res[resb] |= results[key]
            except KeyError:
                return_res[resb] = results[key]
        return return_res
    ...
    def compute_joint_occupancy(self, use_filtered=True):
        ...
        combined_occupancy = _np.ones(self.nb_frames, dtype=_np.bool)
        for res in results:
            combined_occupancy &= results[res]
    ...
    def compute_centrality(
        ...
        for i in range(self.nb_frames):
            ...
            if centrality_type == "betweenness":
                centrality_i = _nx.betweenness_centrality(g_i, normalized=normalize)
            elif centrality_type == "degree":
                centrality_i = _nx.degree_centrality(g_i)
            else:
                raise AssertionError(
                    "centrality_type has to be 'betweenness' or 'degree'"
                )
            for node in centrality_i:
                if (centrality_type == "degree") and not normalize:
                    centralities[node][i] = centrality_i[node] * (
                        g_i.number_of_nodes() - 1
                    )
                else:
                    centralities[node][i] = centrality_i[node]
        if weight is None:
            for node in centralities:
                centralities[node] = _np.round(centralities[node].mean(), 2)
        else:
            ma = max(centralities.values())
            for node in centralities:
                centralities[node] = _np.round(
                    (centralities[node].mean() / ma) / weight, 2
                )
    ...
    def compute_average_position_of_connection(self, use_filtered=True):
        ...
        if self.residuewise:
            all_id = _np.array(self._all_ids + self._ions_ids)
        else:
            all_id = _np.array(self._all_ids_atomwise + self._ions_ids_atomwise)
        all_coordinates = _np.vstack(
            (
                self._da_selection.positions,
                self._water.positions,
                _np.array(self._ions.positions).reshape((-1, 3)),
            )
        )
        residues = _np.array(
            [(key.split(":")[0], key.split(":")[1]) for key in results]
        )
        ...
        for i, ts in enumerate(self._universe.trajectory[self._trajectory_slice]):
            for ii, bond in enumerate(results):
                if results[bond][i]:
                    position[bond] += (
                        all_coordinates[all_id == residues[ii][0]]
                        .reshape((-1, 3))
                        .mean(0)
                        + all_coordinates[all_id == residues[ii][1]]
                        .reshape((-1, 3))
                        .mean(0)
                    ) / 2
        for key in position:
    ...
    def draw_multi_segment_connection_timeseries(
        self,
        segnames=None,
        colors=None,
        use_filtered=True,
        ...
        if segnames == None:
            segnames = _np.unique(
                [
                    [
                        _hf.deconst_key(key, self.residuewise)[0],
                        _hf.deconst_key(key, self.residuewise)[3],
                    ]
                    for key in results
                ]
            )
        ...
        for segnamea, segnameb in combinations(segnames, 2):
            segname_results[segnamea + "-" + segnameb] = {}
        for key in results:
            res1, res2 = key.split(":")
            sn1, sn2 = res1.split("-")[0], res2.split("-")[0]
            comb, comb_check = "-".join((sn1, sn2)), "-".join((sn2, sn1))
            try:
                segname_results[comb][key] = results[key]
            except:
                try:
                    segname_results[comb_check][key] = results[key]
                except:
                    try:
                        segname_results[sn1][key] = results[key]
                    except:
                        try:
                            segname_results[sn2][key] = results[key]
                        except:
                            pass
        ...
        for i, seg in enumerate(segname_results):
            ...
            for bond in segname_results[seg]:
                res_temp += segname_results[seg][bond]
            return_string += " ".join(_np.array(res_temp, dtype=_np.str)) + "\n"
            _plt.plot(
                _np.arange(self.nb_frames), res_temp, color=colors[seg], label=seg
            )
        _plt.legend()
        if return_figure:
            _plt.close()
    ...
    def draw_multi_segname_occupancy_histogram(
        self,
        min_occupancy=0.0,
        max_occupancy=1.0,
        occupancy_step=0.1,
        ...
        segname_results = {segname: {} for segname in segnames}
        temp = []
        for segnamea, segnameb in combinations(segname_results, 2):
            temp.append(segnamea + "-" + segnameb)
        for comb in temp:
            segname_results[comb] = {}
        for key in results:
            res1, res2 = key.split(":")
            sn1, sn2 = res1.split("-")[0], res2.split("-")[0]
            comb, comb_check = "-".join((sn1, sn2)), "-".join((sn2, sn1))
        ...
        return_string = (
            "occupancies "
            + " ".join(_np.array(occupancies.round(2), dtype=_np.str))
            + "\n"
        )
        for i, segname in enumerate(segnames):
            try:
                res[i] = _np.array(
                    [
                        len(_hf.filter_occupancy(segname_results[segname], occupancy))
                ...
                res[i] = 0
            return_string += (
                segname + " " + " ".join(_np.array(res[i], dtype=_np.str)) + "\n"
            )
            handles.append(
                _plt.bar(
                    range(len(res[i])),
                    ...
                    width=0.8,
                    label=segname,
                    color=colors[segname],
                    bottom=res[:i].sum(0),
                )
            )
        for segname in segname_results:
            if segname in segnames:
                continue
            i += 1
            try:
                res[i] = _np.array(
    ...
    def draw_sum_of_connections_timeseries(
        self,
        compare_to=None,
        legend_text=None,
        use_filtered=True,
        ...
        if compare_to != None:
            if use_filtered:
                results_compare = compare_to.filtered_results
            else:
                results_compare = compare_to.initial_results
        ...
        if legend_text != None:
            ...
            return_string += (
                legend_text[0].replace(" ", "_")
                + " ".join(_np.array(res, dtype=_np.str))
                + "\n"
            )
            if compare_to != None:
                ...
                return_string += (
                    legend_text[0].replace(" ", "_")
                    + " ".join(_np.array(res_c, dtype=_np.str))
                    + "\n"
                )
            _plt.legend()
        else:
            ...
            if compare_to != None:
                ...
                return_string += (
                    "compared_connections_per_frame "
                    + " ".join(_np.array(res_c, dtype=_np.str))
                    + "\n"
                )
    ...
    def draw_connections_per_residue(
        self,
        average=False,
        residues_to_plot=None,
        compare_to=None,
        xtick_resnames=True,
        legend_text=None,
        segnames=None,
        ...
        residues = _np.unique(
            [_hf.deconst_key(key, self.residuewise)[2] for key in results]
            + [_hf.deconst_key(key, self.residuewise)[5] for key in results]
        )
        if average:
            results_per_residue = {
                segname: _odict({i: _np.zeros(self.nb_frames) for i in residues})
                for segname in segnames
            }
        else:
            results_per_residue = {
                segname: _odict({i: 0 for i in residues}) for segname in segnames
            }
        if compare_to != None:
            if use_filtered:
                results_compare = compare_to.filtered_results
                ...
                results_compare = compare_to.initial_results
            residues_compare = _np.unique(
                [
                    _hf.deconst_key(key, compare_to.residuewise)[2]
                    for key in results_compare
                ]
                + [
                    _hf.deconst_key(key, compare_to.residuewise)[5]
                    for key in results_compare
                ]
            )
            for key in results_compare:
                _, resna, resia, _, resnb, resib = _hf.deconst_key(
                    key, compare_to.residuewise
                )
                resnames[resia] = resna
                resnames[resib] = resnb
            segnames_compare = _np.unique(
                [
                    [
                        _hf.deconst_key(key, compare_to.residuewise)[0],
                        _hf.deconst_key(key, compare_to.residuewise)[3],
                    ]
                    for key in results_compare
                ]
            )
            if average:
                results_per_residue_compare = {
                    segname: _odict(
                        {i: _np.zeros(compare_to.nb_frames) for i in residues_compare}
                    )
                    for segname in segnames_compare
                }
            else:
                results_per_residue_compare = {
                    segname: _odict({i: 0 for i in residues_compare})
                    for segname in segnames_compare
                }
        for key in results:
            segna, resna, resida, segnb, resnb, residb = _hf.deconst_key(
                key, self.residuewise
            )
            if resna not in ["TIP3", "HOH"] + self._ions_list:
                if average:
                    results_per_residue[segna][resida] += results[key]
                else:
                    results_per_residue[segna][resida] += 1
            if resnb not in ["TIP3", "HOH"] + self._ions_list:
                if average:
                    results_per_residue[segnb][residb] += results[key]
                else:
                    results_per_residue[segnb][residb] += 1
        if average:
            for segn in results_per_residue:
                for key in results_per_residue[segn]:
                    results_per_residue[segn][key] = results_per_residue[segn][
                        key
                    ].mean()
        if compare_to != None:
            for key in results_compare:
                ...
                if resna not in ["TIP3", "HOH"] + self._ions_list:
                    if average:
                        results_per_residue_compare[segna][resida] += results_compare[
                            key
                        ]
                    else:
                        results_per_residue_compare[segna][resida] += 1
                if resnb not in ["TIP3", "HOH"] + self._ions_list:
                    if average:
                        results_per_residue_compare[segnb][residb] += results_compare[
                            key
                        ]
                    else:
                        results_per_residue_compare[segnb][residb] += 1
            if average:
                for segn in results_per_residue_compare:
                    for key in results_per_residue_compare[segn]:
                        results_per_residue_compare[segn][key] = (
                            results_per_residue_compare[segn][key].mean()
                        )
        ...
        if compare_to != None:
            ...
            for segn in all_segnames:
                if segn not in results_per_residue_compare:
                    results_per_residue_compare[segn] = {i: 0 for i in all_residues}
                if segn not in results_per_residue:
                    results_per_residue[segn] = {i: 0 for i in all_residues}
                for key in all_residues:
                    if key not in results_per_residue[segn]:
                        results_per_residue[segn][key] = 0
                    if key not in results_per_residue_compare[segn]:
                        results_per_residue_compare[segn][key] = 0

            for segn in results_per_residue:
                results_per_residue[segn] = _odict(
                    {
                        key: results_per_residue[segn][key]
                        for key in sorted(list(results_per_residue[segn].keys()))
                    }
                )
            for segn in results_per_residue_compare:
                results_per_residue_compare[segn] = _odict(
                    {
                        key: results_per_residue_compare[segn][key]
                        for key in sorted(
                            list(results_per_residue_compare[segn].keys())
                        )
                    }
                )
            ys_compare = {segn: [] for segn in results_per_residue}

        xs, ys = {segn: [] for segn in results_per_residue}, {
            segn: [] for segn in results_per_residue
        }
        for segn in results_per_residue:
            for key in results_per_residue[segn]:
                xs[segn].append(key)
        ...
        if compare_to != None:
            ys_compare_bottom[ss[0]] = _np.zeros(len(ys_compare[ss[0]]))
        for i in range(1, len(ss)):
            ys_bottom[ss[i]] = ys[ss[i - 1]]
            if compare_to != None:
                ys_compare_bottom[ss[i]] = ys_compare[ss[i - 1]]
        fig, ax = _plt.subplots()
        for segn in xs:
            ...
            if compare_to != None:
                if legend_text != None:
                    if ys[segn].sum() > 0:
                        _plt.bar(
                            _np.arange(len(ys[segn])) - 0.1,
                            ys[segn],
                            width=0.2,
                            label=legend_text[0] + " - " + segn,
                            bottom=ys_bottom[segn],
                        )
                    if ys_compare[segn].sum() > 0:
                        _plt.bar(
                            _np.arange(len(ys_compare[segn])) + 0.1,
                            ys_compare[segn],
                            width=0.2,
                            label=legend_text[1] + " - " + segn,
                            bottom=ys_compare_bottom[segn],
                        )
                    _plt.legend()
                else:
                    _plt.bar(
        ...
        all_labels = [
            (
                _hf.aa_three2one[resnames[resi]] + str(resi + self.add_missing_residues)
                if resnames[resi] in _hf.aa_three2one
                else resnames[resi] + str(resi + self.add_missing_residues)
            )
            for resi in all_xs
        ]
        ...
        for segn in xs:
            return_string += (
                segn + " " + " ".join(_np.array(ys[segn], dtype=_np.str)) + "\n"
            )
        if compare_to != None:
            for segn in xs:
                return_string += (
                    ...
                    + segn
                    + " "
                    + " ".join(_np.array(ys_compare[segn], dtype=_np.str))
                    + "\n"
                )
    ...
    def draw_joint_timeseries(
        self, use_filtered=True, scatter_size=0.5, filename=None, return_figure=False
    ):
        if use_filtered:
            results = self.filtered_results
        else:
        ...
        ax.xaxis.set_major_locator(MaxNLocator(integer=True))
        return_string = "Joint_occupancy " + " ".join(
            _np.array(ts.astype(_np.int), dtype=_np.str)
        )
        if return_figure:
            _plt.close()
            return fig, _hf.string_in_columns(return_string)
    ...
    def draw_compare_joint_timeseries(
        self,
        other_paths,
        descriptor="path",
        scatter_size=0.5,
        ...
        for other in other_paths:
            if use_filtered:
                results.append(other.filtered_results)
            else:
                results.append(other.initial_results)
        ...
        for i, ind in enumerate(sorted_index):
            _plt.scatter(
                range(len(ts[ind])),
                _np.array(ts[ind], dtype=int) * (i + 1),
                s=scatter_size,
            )
        _plt.xlabel("Frame", fontsize=16)
        ax.set_xlim(0, self.nb_frames - 1)
        ...
        ax.set_yticklabels(
            ["{}".format(o * 100) for o in occupancies[sorted_index].round(1)]
        )
        ax.set_ylabel("Joint Occupancy [%]", fontsize=16)
        ax2.set_yticks(range(1, len(ts) + 1))
        ax2.set_yticklabels(
            [(descriptor + " {}").format(i + 1) for i in range(len(ts))]
        )
        ...
        for i, ind in enumerate(sorted_index):
            return_string += (
                ...
                + str(i + 1)
                + " "
                + " ".join(_np.array(ts[ind].astype(_np.int), dtype=_np.str))
                + "\n"
            )
        if return_figure:
            _plt.close()
    ...
    def draw_occupancy_histogram(
        ...
        occupancies = _np.arange(min_occupancy, max_occupancy, occupancy_step)
        bond_counts = [
            len(_hf.filter_occupancy(results, occupancy)) for occupancy in occupancies
        ]
        ...
        if compare_to != None:
            diff_counts_c = _np.array(
                [
                    len(_hf.filter_occupancy(results_compare, occupancy))
                    for occupancy in occupancies
                ]
            )
            if legend_text != None:
                _plt.bar(
                    _np.array(list(range(len(diff_counts)))) - 0.2,
                    diff_counts,
                    width=0.4,
                    label=legend_text[0],
                )
                ...
                return_string += (
                    legend_text[0]
                    + " "
                    + " ".join(_np.array(diff_counts, dtype=_np.str))
                    + "\n"
                )
                return_string += (
                    legend_text[1]
                    + " "
                    + " ".join(_np.array(diff_counts_c, dtype=_np.str))
                    + "\n"
                )
                _plt.legend()
            else:
    ...
    def draw_compare_occupancy_histogram(
        self,
        compare_to,
        xaxis_names=None,
        use_filtered=True,
        ...
        if xaxis_names != None:
            ...
            labels = _np.arange(1, len(results) + 1)
        fig, ax = _plt.subplots()
        ax.spines["right"].set_visible(False)
        ax.spines["top"].set_visible(False)
    ...
    def draw_time_histogram(
        ...
        if (last_frame < 0) or (last_frame > self.nb_frames):
            last = self.nb_frames
        else:
            last = last_frame
        block_values = _np.linspace(first_frame, last, nb_blocks, dtype=int)
        y_hist = []
        for i, ii in _hf.pairwise(block_values):
            y_hist.append(values[:, i:ii].sum())
        if compare_to != None:
            ...
            y_hist_c = []
            for i, ii in _hf.pairwise(block_values):
                y_hist_c.append(values[:, i:ii].sum())
        labels = ["{}".format(block_value) for block_value in block_values]
        fig, ax = _plt.subplots()
        ax.spines["right"].set_visible(False)
        ax.spines["top"].set_visible(False)
        return_string = (
            "frame_from " + " ".join([labels[i] for i in range(len(labels) - 1)]) + "\n"
        )
        return_string += (
            "frame_to "
            + " ".join([labels[i + 1] for i in range(len(labels) - 1)])
            + "\n"
        )
        if compare_to != None:
            if legend_text != None:
                _plt.bar(
                    _np.array(list(range(len(y_hist)))) + 0.3,
                    y_hist,
                    width=0.4,
                    label=legend_text[0],
                )
                _plt.bar(
                    _np.array(list(range(len(y_hist_c)))) + 0.7,
                    y_hist_c,
                    width=0.4,
                    label=legend_text[1],
                )
                return_string += (
                    legend_text[0]
                    + " "
                    + " ".join(_np.array(y_hist, dtype=_np.str))
                    + "\n"
                )
                return_string += (
                    legend_text[1]
                    + " "
                    + " ".join(_np.array(y_hist_c, dtype=_np.str))
                    + "\n"
                )
                _plt.legend()
            else:
                ...
                _plt.bar(
                    _np.array(list(range(len(y_hist_c)))) + 0.7, y_hist_c, width=0.4
                )
                return_string += (
                    "main" + " " + " ".join(_np.array(y_hist, dtype=_np.str)) + "\n"
                )
                return_string += (
                    "compare"
                    + " "
            ...
            if legend_text != None:
                _plt.bar(
                    _np.array(list(range(len(y_hist)))) + 0.5,
                    y_hist,
                    label=legend_text[0],
                )
                return_string += (
    ...
    def draw_residue_range_heatmap(
        self,
        ranges,
        names,
        average=False,
        ...
        residues = []
        for r in ranges:
            residues += list(range(r[0], r[1] + 1))
        residues = _np.array(residues)
        residue_trans = _np.empty(_np.max(ranges) + 1, dtype=int)
        for i in range(len(ranges)):
            residue_trans[ranges[i][0] : ranges[i][1] + 1] = i
        if average:
            results_dict = {
                i: {j: _np.zeros(self.nb_frames) for j in range(len(ranges))}
                for i in range(len(ranges))
            }
        ...
        for key in results:
            ...
            if (
                resna not in ["TIP3", "HOH"]
                and resnb not in ["TIP3", "HOH"]
                and resida in residues
                and residb in residues
            ):
                if average:
                    results_dict[residue_trans[resida]][
                        residue_trans[residb]
                    ] += results[key]
                else:
                    results_matrix[residue_trans[resida], residue_trans[residb]] += 1
        if average:
            for i in range(len(ranges)):
        ...
        result_string += (
            "resid_from_to "
            + " ".join([str(r[0]) + "-" + str(r[1]) for r in ranges])
            + "\n"
        )
        result_string = (
            _hf.string_in_columns(result_string) + " \n\nblock_1 block_2 count\n"
        )
        for i, row in enumerate(results_matrix):
            for j in range(i, len(results_matrix)):
                result_string += (
                    ...
                    + names[j]
                    + " "
                    + str(results_matrix[i, j].round(2))
                    + "\n"
                )
        _plt.setp(
            ax.get_yticklabels(),
    ...
    def draw_residue_residue_heatmap(
        self, average=False, use_filtered=True, filename=None, return_figure=False
    ):
        if use_filtered:
            results = self.filtered_results
        else:
        ...
        if average:
            results_average = _np.zeros((len(residues), len(residues), self.nb_frames))
        for key in results:
            _, resna, resida, _, resnb, residb = _hf.deconst_key(key, self.residuewise)
            if resida in residues and residb in residues:
                resmin, resmax = min(res_trans[resida], res_trans[residb]), max(
                    res_trans[resida], res_trans[residb]
                )
                if average:
                    results_average[resmin, resmax] += results[key]
                else:
                    results_matrix[resmin, resmax] += 1
        if average:
            results_matrix = results_average.mean(2).round(2)
        results_matrix = results_matrix + _np.tril(results_matrix.T, -1)
        if not average:
            results_matrix = results_matrix.astype(_np.int)
        fig, ax = _plt.subplots()
        ...
        for i in range(len(ida)):
            ...
            result_string += (
                ...
                + names[idb[i]]
                + " "
                + str(results_matrix[ida[i], idb[i]])
                + "\n"
            )
        if return_figure:
            _plt.close()
    ...
    def draw_graph(
        self,
        draw_edge_occupancy=True,
        compare_to=None,
        mutations=None,
        highlight_interregion=False,
        in_frame=None,
        centrality=None,
        max_centrality=None,
        ...
        if compare_to is not None:
            ...
            if mutations is not None:
                for mutation in mutations:
                    ...
                    temp_edges = [
                        (new_node, connected_node)
                        for connected_node in graph1[old_node]
                    ]
        ...
        if in_frame is None:
            ...
            if self.joint_occupancy_frames.size > 0:
                in_frame = self.joint_occupancy_frames[0]
            else:
                in_frame = 0
        ...
        for node in nodes:
            if node in new_nodes:
                node = old_nodes[new_nodes.index(node)]
        ...
        if draw_base:
            ...
            ax_label_pos = (
                arrow_pos
                + arrow_pos / _np.linalg.norm(arrow_pos, axis=1).reshape(-1, 1) * 0.5
            )
        ...
        if highlight_interregion:
            ...
            _nx.draw_networkx_edges(interregion_graph, pos, width=10.0, alpha=0.7)
        if compare_to != None:
            _nx.draw_networkx_edges(
                edges_gone, pos, width=3.5, alpha=0.7, edge_color="red"
            )
            _nx.draw_networkx_edges(
                edges_new, pos, width=3.5, alpha=0.7, edge_color="green"
            )
        labels = {}
        ...
        if color_dict:
            ...
            segnames = _np.unique(
                _np.array(
                    [
                        [
                            bond.split(":")[0].split("-")[0],
                            bond.split(":")[1].split("-")[0],
                        ]
                        for bond in results
                    ]
                )
        ...
        for j, node in enumerate(composed_graph.nodes()):
            if self.residuewise:
                ...
                segname, resname, resid, atomname = node.split("-")
            try:
                if self.residuewise:
                    if resname == "TIP3":
                        labels[node] = segname + str(
                            int(resid) + self.add_missing_residues
                        )
                    else:
                        labels[node] = _hf.aa_three2one[resname] + str(
                            int(resid) + self.add_missing_residues
                        )
                else:
                    if resname == "TIP3":
                        labels[node] = (
                            segname
                            + str(int(resid) + self.add_missing_residues)
                            + "\n"
                            + atomname
                        )
                    else:
                        labels[node] = (
                            _hf.aa_three2one[resname]
                            + str(int(resid) + self.add_missing_residues)
                            + "\n"
                            + atomname
                        )
        ...
        if centrality is not None:
            color_value_list = [
                centrality[node] if node in centrality else 0
                for node in composed_graph.nodes()
            ]
            if max_centrality is not None:
                mc = max_centrality
            else:
                mc = max(color_value_list)
            ...
            for color in color_graph:
                _nx.draw_networkx_nodes(
                    color_graph[color], pos, node_size=900, alpha=0.5, node_color=color
                )
        # f = _np.sqrt(_np.sqrt(_np.sqrt(_np.sqrt(len(pos)))))*0.5*(1 - 0.5 * (1 - node_factor))
        f = 0.6
        len2fontsize = _hf.defaultdict(
            lambda: 6 * f,
            {
                2: 18 * f,
                3: 18 * f,
        ...
        for j, node in enumerate(composed_graph.nodes()):
            ...
            if len(spl) > 1:
                label_length = len(spl[0]) + 1
            else:
                label_length = len(spl[0])
            if draw_labels:
        ...
        if draw_edge_occupancy:
            ...
            if compare_to != None:
                ...
                for conn in compare_results:
                    nodea, nodeb = conn.split(":")
                    compare_edge_labels[(nodea, nodeb)] = _np.round(
                        compare_results[conn].mean() * 100, 1
                    )
                ...
                _nx.draw_networkx_edge_labels(
                    composed_graph,
                    pos,
                    edge_labels=compare_edge_labels,
                    font_size=len2fontsize[4],
                    label_pos=0.66,
                    font_color="green",
                )
    ...
    def _reload_universe(self):
        ...
        sorted_selection = _hf.Selection(
            ...
            self.donor_names,
            self.acceptor_names,
            self._add_donors_without_hydrogen,
        )
        if not sorted_selection.donors:
            da_selection = sorted_selection.acceptors
        elif not sorted_selection.acceptors:
    ...
    def _compute_graph_in_frame(
        self, frame, set_as_filtered_results=False, use_filtered=True
    ):
        if use_filtered:
            results = self.filtered_results
        else:
    ...
    def _generate_current_results_from_graph(self):
        ...
        for resa, resb in self.initial_graph.edges():
            ...
            try:
                temp_res[key] = self.initial_results[key]
            except KeyError:
                temp_res[key_check] = self.initial_results[key_check]
