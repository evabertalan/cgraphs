...
class HydrationAnalysis(BasicFunctionality):
    ...
    def __init__(
        ...
        super(HydrationAnalysis, self).__init__(
            selection=selection,
            structure=structure,
    ...
    def set_presence_around(self, around_distance, per_residue=False):
        ...
        for ts in self._universe.trajectory[self._trajectory_slice]:
            ...
            [
                ...
                for l in selection_tree.query_ball_tree(
                    water_tree, float(around_distance)
                )
            ]
            ...
            if per_residue:
                ...
                local_pairs = [
                    (i, local_water_index[j] + _first_water_id)
                    for i, bla in enumerate(
                        selection_tree.query_ball_tree(
                            local_water_tree, around_distance
                        )
                    )
                    for j in bla
                ]
                check = [
                    _all_ids[j] + ":" + _all_ids[i] in result for (i, j) in local_pairs
                ]
                frame_res = [
                    (
                        _all_ids[j] + ":" + _all_ids[i]
                        if check[ii]
                        else _all_ids[i] + ":" + _all_ids[j]
                    )
                    for ii, (i, j) in enumerate(local_pairs)
                ]
            else:
                frame_res = [self._water_ids[i] for i in local_water_index]
    ...
    def compute_diffusion_coefficient(
        self, fit_begin=0.1, fit_end=0.9, nb_blocks=1, use_filtered=True
    ):
        msd, err = self.msd
        ...
        popt, pcov = _curve_fit(
            func, _np.arange(block_length)[cut_begin:cut_end], msd[cut_begin:cut_end]
        )
        return popt[0] / 6.0
    ...
    def _compute_mean_square_displacement_slow(self, nb_blocks=1, use_filtered=True):
        ...
        water_coords = _np.empty(
            (self.nb_frames, water_index.sum(), 3), dtype=_np.float16
        )
        for i, t in enumerate(self._universe.trajectory[self._trajectory_slice]):
            water_coords[i] = water_involved.positions
        all_msd = []
    ...
    def compute_mean_residence_time(
        self,
        nb_blocks=1,
        ...
        for run in range(nb_blocks):
            ...
            average_time = _np.array([0.0, 0.0])
            ...
            if per_residue:
                ...
                for pair in results:
                    ...
                    interval_lengths = (
                        _np.diff(work_intervals, 1).astype(_np.int).flatten()
                    )
                    if filter_artifacts:
                        interval_lengths = interval_lengths[
                            (interval_lengths < block_length)
                        ]
                    try:
                ...
                for residue_key in residence_time:
                    ...
                    average_time[residue_key] = (
                        average_time[residue_key][0] / average_time[residue_key][1]
                    )
                    try:
                        (tau, lamda), pcov = _curve_fit(
                            func, xdata, residence_time[residue_key], p0=(10.0, 0.5)
                        )
                ...
                try:
                    (tau, lamda), pcov = _curve_fit(
                        func, xdata, residence_time, p0=(10.0, 0.5)
                    )
            ...
            for key in residence_time:
                ...
                for average_dict in all_average_time:
                    try:
                        ...
                        pass
                residence_time[key] = (
                    _np.mean(temp_residence_time),
                    _np.std(temp_residence_time) / _np.sqrt(nb_blocks),
                )
                average_time[key] = (
                    _np.mean(temp_average_time),
                    _np.std(temp_average_time) / _np.sqrt(nb_blocks),
                )
        else:
            residence_time = (
                _np.mean(all_residence_time),
                _np.std(all_residence_time) / _np.sqrt(nb_blocks),
            )
            average_time = (
                _np.mean(all_average_time),
                _np.std(all_average_time) / _np.sqrt(nb_blocks),
            )

        if return_average_time:
            return residence_time, average_time
        else:
            return residence_time

    def draw_mean_residence_time_per_residue(
        self,
        resids_to_plot,
        nb_blocks=1,
        filter_artifacts_above=None,
        filename=None,
        return_figure=False,
    ):
        residence_time = self.compute_mean_residence_time(
            nb_blocks=nb_blocks, per_residue=True
        )
        ...
        segnames, resnames, resids, residence_time, errors = (
            ...
            _np.array(resids)[index],
            _np.array(residence)[index],
            _np.array(errors)[index],
        )
        ...
        segnames, resnames, resids, residence_time, errors = (
            ...
            _np.array(resnames)[index],
            _np.array(resids)[index],
            _np.array(residence_time)[index],
            _np.array(errors)[index],
        )
        u_segnames = _np.unique(segnames)
        per_segment = {seg: [] for seg in u_segnames}
        per_segment_error = {seg: [] for seg in u_segnames}
        for i in range(len(residence_time)):
            per_segment[segnames[i]].append(residence_time[i])
        for i in range(len(residence_time)):
            per_segment_error[segnames[i]].append(errors[i])
        ...
        if u_segnames.size == 1:
            _plt.bar(_np.arange(len(residence_time)), residence_time, yerr=errors)
        else:
            ...
            for i in range(len(u_segnames)):
                ...
                per_segment[u_segnames[i]][
                    per_segment[u_segnames[i]] > self.nb_frames
                ] = 0
                _plt.bar(
                    _np.arange(len(per_segment[u_segnames[i]])) + centers[i],
                    per_segment[u_segnames[i]],
                    yerr=per_segment_error[u_segnames[i]],
                    width=width,
                    label=u_segnames[i],
                )
            _plt.legend()
    ...
    def draw_mean_residence_time_histogram(
        self, nb_bins=50, filter_lower_than=None, filename=None, return_figure=False
    ):
        ...
        if filter_lower_than != None:
            residence_time = residence_time[residence_time < filter_lower_than]
        fig, ax = _plt.subplots()
        ax.spines["right"].set_visible(False)
        ax.spines["top"].set_visible(False)
    ...
    def compute_water_count(self, use_filtered=True):
        if use_filtered:
            water_presence = self.filtered_results
        else:
            water_presence = self.initial_results
    ...
    def compute_influx_count(self, return_mean_and_std=False, use_filtered=True):
        ...
        for frame in range(self.nb_frames - 1):
            results[0][frame] = (
                _np.logical_not(presence[frame]) & presence[frame + 1]
            ).sum()
            results[1][frame] = (
                presence[frame] & _np.logical_not(presence[frame + 1])
            ).sum()
        if return_mean_and_std:
            return results, results.mean(1), results.std(1)
        else:
            return results

    def draw_influx_count(
        self, interval=1, use_filtered=True, filename=None, return_figure=False
    ):
        results = self.compute_influx_count(use_filtered=use_filtered)
        rest = (results[0].size % interval) * -1
        if rest != 0:
            results = results.T[:rest].T
