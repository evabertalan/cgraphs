...
class HbondAnalysis(NetworkAnalysis):
    ...
    def __init__(
        ...
        super(HbondAnalysis, self).__init__(
            selection=selection,
            structure=structure,
            ...
            exclude_acceptors=exclude_acceptors,
            ions=ions,
    ...
    def set_add_ion_interactions(self, include_water=True):
        if not self._ions:
            raise AssertionError("No ions were specified during initialization.")
        frame_count = 0
        ...
        for ts in self._universe.trajectory[self._trajectory_slice]:
            ...
            da_ion_pairs = _np.array(
                [
                    [i, j]
                    for i, ions in enumerate(
                        selection_tree.query_ball_tree(ion_tree, self.distance)
                    )
                    for j in ions
                ]
            )
            ...
            if include_water:
                ...
                water_ion_pairs = _np.array(
                    [
                        [i, j]
                        for i, ions in enumerate(
                            water_tree.query_ball_tree(ion_tree, self.distance)
                        )
                        for j in ions
                    ]
                )
                water_frame_res = [
                    water_ids[i] + ":" + ion_ids[j] for i, j in water_ion_pairs
                ]
        ...
        if not self.initial_results:
            ...
            self._add_overwrite_results(result)
    ...
    def set_hbonds_in_selection(self, exclude_backbone_backbone=True):
        ...
        frames = self.nb_frames
        result = {}
        if self._nb_acceptors == 0:
            raise AssertionError("No acceptors in the selection")
        if self._nb_donors == 0:
            raise AssertionError("No donors in the selection")
        for ts in self._universe.trajectory[self._trajectory_slice]:
            selection_coordinates = self._da_selection.positions
            ...
            frame_res = [
                (
                    self._all_ids[i] + ":" + self._all_ids[j]
                    if check[ii]
                    else self._all_ids[j] + ":" + self._all_ids[i]
                )
        ...
        self._set_results(result)
        if self._ions_list:
            self.set_add_ion_interactions(False)
    ...
    def set_hbonds_only_water_in_convex_hull(self):
        ...
        for ts in self._universe.trajectory[self._trajectory_slice]:
            ...
            water_pairs = local_index[
                _np.array(
                    [pair for pair in local_water_tree.query_pairs(self.distance)]
                )
            ]

            if self.check_angle:
                local_hbonds = _hf.check_angle_water(
                    water_pairs, water_coordinates, hydrogen_coordinates, self.cut_angle
                )
            else:
            ...
            if self.residuewise:
                ...
                frame_res = [
                    (
                        ...
                        if check[ii]
                        else self._all_ids_atomwise[j] + ":" + self._all_ids_atomwise[i]
                    )
                    for ii, (i, j) in enumerate(sorted_bonds)
                ]
    ...
    def set_hbonds_in_selection_and_water_in_convex_hull(
        self, exclude_backbone_backbone=True
    ):
        if exclude_backbone_backbone:
            backbone_filter = _np.array(
                [(ids.split("-")[3] in ["O", "N"]) for ids in self._all_ids_atomwise]
        ...
        for ts in self._universe.trajectory[self._trajectory_slice]:
            ...
            hull = _sp.Delaunay(select_coordinates)
            local_water_index = (hull.find_simplex(water_coordinates) != -1).nonzero()[
                0
            ]
            ...
            local_pairs = [
                (i, local_water_index[j] + self._first_water_id)
            ...
            water_pairs = [
                (
                    local_water_index[p[0]] + self._first_water_id,
        ...
        self._set_results(result)
        if self._ions_list:
            self.set_add_ion_interactions(True)

    def set_hbonds_in_selection_and_water_around(
        self, around_radius, exclude_backbone_backbone=True
    ):
        if exclude_backbone_backbone:
            backbone_filter = _np.array(
                [(ids.split("-")[3] in ["O", "N"]) for ids in self._all_ids_atomwise]
        ...
        for ts in self._universe.trajectory[self._trajectory_slice]:
            ...
            [
                ...
                for l in water_tree.query_ball_point(
                    selection_coordinates, float(around_radius)
                )
            ]
            ...
            if self._nb_acceptors > 0 and self._nb_donors > 0:
                ...
                da_pairs = []
            if exclude_backbone_backbone and da_pairs != []:
                da_pairs = da_pairs[
                    _np.logical_not(_np.all(backbone_filter[da_pairs], axis=1))
                ]
    ...
    def filter_only_backbone_bonds(self, use_filtered=True):
        assert (
            self.residuewise == False
        ), "residuewise has to be False for this filter to work."
        if use_filtered:
        ...
        atomname_pairs = _np.array(
            [
                [bond.split(":")[0].split("-")[3], bond.split(":")[1].split("-")[3]]
                for bond in results
            ]
        )
        backbone_backbone_filter = _np.array(
            [
                (name[0] in ["O", "N"] and name[1] in ["O", "N"])
                for name in atomname_pairs
            ]
        )
        self.filtered_results = {
            key: results[key]
            for i, key in enumerate(results)
            if backbone_backbone_filter[i]
        }
    ...
    def filter_water_in_hydration_shells(self, hydration_shells=3, use_filtered=True):
        ...
        hysh_lengths = []
        res_nodes = [
            node for node in graph.nodes() if node.split("-")[1] not in ["TIP3", "HOH"]
        ]
        for res_node in res_nodes:
            hysh_lengths += _nx.single_source_shortest_path_length(
                graph, res_node, hydration_shells
            )
        ...
        for edge in graph.edges():
            ...
            check = edge[1] + ":" + edge[0]
            try:
                filtered_result[bond_name] = results[bond_name]
            except KeyError:
                filtered_result[check] = results[check]
        if self.applied_filters["shells"] != None:
            self.applied_filters["shells"] = _np.min(
                self.applied_filters["shells"], hydration_shells
            )
        else:
            self.applied_filters["shells"] = hydration_shells
    ...
    def compute_i4_bond_lengths(
        self,
        filter_above_factor=None,
        average_over_segments=True,
        print_table=True,
        use_filtered=True,
    ):
        if self.residuewise:
            raise AssertionError("need to initialize with residuewise=False!")
        if use_filtered:
            results = self.filtered_results
        else:
            results = self.initial_results
        ...
        resid_pairs = _np.array(
            [
                [bond.split(":")[0].split("-")[2], bond.split(":")[1].split("-")[2]]
                for bond in results
            ],
            dtype=_np.int,
        )
        ...
        keypairs = _np.array(
            [[bond.split(":")[0], bond.split(":")[1]] for bond in results]
        )[i4_filter]
        keyindex = _np.array(
            [
                [_np.where(ids == keypair[0])[0][0], _np.where(ids == keypair[1])[0][0]]
        ...
        if average_over_segments:
            ...
            resnames_average = _np.array(
                [
                    [bond.split(":")[0].split("-")[1], bond.split(":")[1].split("-")[1]]
                    for bond in results
                ]
            )[i4_filter][resid_sort_index]
            ...
            for i in range(len(l)):
                ...
                if list(resids_average[i]) not in already_checked:
                    l_average.append(
                        resnames_average[i][0]
                        + "-"
                        + str(resids_average[i][0])
                        + "-"
                        + atomname_average[i][0]
                        + ":"
                        + resnames_average[i][1]
                        + "-"
                        + str(resids_average[i][1])
                        + "-"
                        + atomname_average[i][1]
                    )
                    dists_average.append(
                        dists[(resids_average == resids_average[i]).all(1)].mean()
                    )
    ...
    def compute_i4_bonds(
        self, use_filtered=True, return_print_table=False, print_table=False
    ):
        if self.residuewise:
            raise AssertionError("need to initialize with residuewise=False!")
        if use_filtered:
        ...
        segname_pairs = _np.array(
            [
                [bond.split(":")[0].split("-")[0], bond.split(":")[1].split("-")[0]]
                for bond in results
            ]
        )
        atomname_pairs = _np.array(
        ...
        ser_thr_filter = _np.array(
            [
                (ids[0] in ["SER", "THR"]) or (ids[1] in ["SER", "THR"])
                for i, ids in enumerate(resnames_pairs)
            ]
        )
        glu_asp_filter = _np.array(
            [
                (ids[0] in ["GLU", "ASP"]) or (ids[1] in ["GLU", "ASP"])
                for i, ids in enumerate(resnames_pairs)
            ]
        )
        backbone_filter = _np.array(
            [
                not (name[0] in ["O", "N"] or name[1] in ["O", "N"])
                for name in atomname_pairs
            ]
        )
        backbone_backbone_filter = _np.array(
            [
                not (name[0] in ["O", "N"] and name[1] in ["O", "N"])
                for name in atomname_pairs
            ]
        )
        is_backbone = _np.array(
            [[pair[0] in ["O", "N"], pair[1] in ["O", "N"]] for pair in atomname_pairs],
            dtype=_np.bool,
        )
        both_filter = ser_thr_filter & glu_asp_filter & backbone_filter
        res_thr_filter = _np.array(
            [
                [pair[0] in ["SER", "THR"], pair[1] in ["SER", "THR"]]
                for pair in resnames_pairs
            ],
            dtype=_np.bool,
        )
        i4_filter = (
        ...
        output = ""
        translate_key = {
            l[i]: segname_pairs[i][0]
            + "-"
            + resnames_pairs[i][0]
            + "-"
            + str(resid_pairs[i][0] + self.add_missing_residues)
            + "-"
            + atomname_pairs[i][0]
            + ":"
            + segname_pairs[i][1]
            ...
            + resnames_pairs[i][1]
            + "-"
            + str(resid_pairs[i][1] + self.add_missing_residues)
            + "-"
            + atomname_pairs[i][1]
        ...
        for i in range(len(l)):
            ...
            if both_filter[i]:
                ...
                other_bond_filter = (
                    (resid_pairs == resid)
                    & (segname_pairs == segname)
                    & (resnames_pairs == resname)
                    & (atomname_pairs == atomname)
                ).any(1) & i4_filter
                for j in _np.nonzero(other_bond_filter)[0]:
                    sel_i, sel_j = _np.logical_not(res_thr_filter[i]), _np.logical_not(
                        res_thr_filter[j]
                    )
                    if not (
                        segname_pairs[i][sel_i] == segname_pairs[j][sel_j]
                        and resnames_pairs[j][sel_j] == resnames_pairs[i][sel_i]
                        and resid_pairs[j][sel_j] == resid_pairs[i][sel_i]
                    ):
        ...
        for comb in temp:
            segname_results[comb] = {1: {}, 2: {}}
        for key in class_dict:
            residue1, residue2 = key.split(":")
            sn1, sn2 = residue1.split("-")[0], residue2.split("-")[0]
            comb, comb_check = "-".join((sn1, sn2)), "-".join((sn2, sn1))
            try:
                segname_results[comb][class_dict[key]][key] = results[key]
            except:
                try:
                    segname_results[comb_check][class_dict[key]][key] = results[key]
                except:
                    try:
                        segname_results[sn1][class_dict[key]][key] = results[key]
                    except:
                        try:
                            segname_results[sn2][class_dict[key]][key] = results[key]
                        except:
                            pass
        for segname in segname_results:
            ...
            for class_id in segname_results[segname]:
                for key in segname_results[segname][class_id]:
                    output += (
                        translate_key[key]
                        + "." * (45 - len(translate_key[key]))
                        + " | "
                        + "class "
                        + str(class_dict[key])
                        + " | "
                        + str(_np.round(results[key].mean() * 100, 1))
                        + "\n"
                    )
                    if key in res3:
                        class3_text += (
                            ...
                            + str(
                                _np.round(
                                    (results[key] & results[res3[key]]).mean() * 100, 1
                                )
                            )
                            + "\n"
                        )
                        class3_text += (
                            translate_key[res3[key]]
                            + "." * (45 - len(translate_key[res3[key]]))
                            + " | "
                            + "class "
                            + str(3)
                            + " | "
    ...
    def compute_i4_motif_distribution(self):
        ...
        for key in class_dict:
            ...
            atoms = self._mda_selection.select_atoms(
                "(segid {} and resid {} and name {}) or (segid {} and resid {} and name {})".format(
                    segida, resida, atoma, segidb, residb, atomb
                )
            )
        ...
        for ts in self._universe.trajectory[self._trajectory_slice]:
            for key in z:
                if not isinstance(z[key], list):
                    if key == 3:
                        z_plot[key].append(
                            z[key].positions[:, 2].reshape(-1, 4).mean(1)
                        )
                    else:
                        z_plot[key].append(
                            z[key].positions[:, 2].reshape(-1, 2).mean(1)
                        )
        z_plot = [
            (
                _np.array(z_plot[key]).mean(0)
                if z_plot[key] != []
                else _np.array(z_plot[key])
            )
            for key in z_plot
        ]
    ...
    def draw_i4_motif_distribution(self, filename=None, return_figure=False):
        ...
        # print(z_plot)
        mi, ma = _np.min([a.min() for a in z_plot if a.size > 0]), _np.max(
            [a.max() for a in z_plot if a.size > 0]
        )
        ...
        return_str2 += (
            "z_intervals "
            + " ".join(
                [interv[i] + "_" + interv[i + 1] for i in range(len(interv) - 1)]
            )
            + "\n"
        )
        return_str2 += (
            "intra-helical "
            + " ".join(_np.histogram(z_plot[0], binedges)[0].astype(_np.str))
            + "\n"
        )
        return_str2 += (
            "inter-helical "
            + " ".join(_np.histogram(z_plot[1], binedges)[0].astype(_np.str))
            + "\n"
        )
        return_str2 += (
            "intra+inter-helical "
            + " ".join(_np.histogram(z_plot[2], binedges)[0].astype(_np.str))
            + "\n"
        )
    ...
    def draw_hbond_layer_occupancy_histogram(
        self,
        min_occupancy,
        occupancy_step,
        use_filtered=True,
        ...
        if len(results) == 0:
            raise AssertionError("nothing to filter!")
        occupancies = _np.arange(min_occupancy, 1.0, occupancy_step)
        bonds_list = [
            _hf.filter_occupancy(results, occupancy) for occupancy in occupancies
        ]
        ...
        for i, occupancy in enumerate(occupancies):
            ...
            res_nodes = [
                ...
                if node.split("-")[1] not in ["TIP3", "HOH"]
            ]
            for res_node in res_nodes:
                hysh_lengths = _nx.single_source_shortest_path_length(
                    graphs[i], res_node, 3
                ...
                for hydration_water in hysh_lengths:
                    if hydration_water.split("-")[1] != "TIP3":
                        continue
                    hysh_shells[hysh_lengths[hydration_water] - 1].append(
                        hash(hydration_water)
                    )
        ...
        ax.spines["top"].set_visible(False)
        second = _plt.bar(
            _np.arange(len(diff_counts[1]), dtype=int) * 4,
            diff_counts[1],
            label="Second Layer",
        )
        _plt.xticks(_np.arange(len(diff_counts[1]), dtype=int) * 4, labels)
        first = _plt.bar(
            _np.arange(len(diff_counts[0]), dtype=int) * 4 - 1,
            diff_counts[0],
            label="First Layer",
        )
        third = _plt.bar(
            _np.arange(len(diff_counts[2]), dtype=int) * 4 + 1,
            diff_counts[2],
            label="Third Layer",
        )
