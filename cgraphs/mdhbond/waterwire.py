...
class WireAnalysis(NetworkAnalysis):
    ...
    def __init__(
        ...
        self.hash_table = {}
        self.wire_lengths = {}
        da_u, da_ind, da_inv = _np.unique(
            da_ids, return_index=True, return_inverse=True
        )
    ...
    def set_water_wires(
        ...
        for ts in self._universe.trajectory[self._trajectory_slice]:
            ...
            if local_water_index.size > 0:
                ...
                try:
                    water_pairs = local_water_index[
                        _np.array(list(local_water_tree.query_pairs(self.distance)))
                    ]
                except IndexError:
            ...
            if local_hbonds.size > 0:
                ...
                for source in residues:
                    ...
                    idx = _np.array(
                        [
                            self._all_ids[bl] != self._all_ids[source]
                            for bl in local_hbonds[:, 0]
                        ]
                    )
                    ...
                    for target, last_water in local_hbonds[all_targets_index]:
                        ...
                        if (
                            target in already_checked
                            or target in already_checked_targets
                            or self._all_ids[source] == self._all_ids[target]
                        ):
                            continue
                        ...
                        if water_in_wire >= water_already_found:
                            continue
    ...
    def set_explicit_water_wires(
        ...
        for wire_key in self.hashs:
            ...
            for i, h in enumerate(hashs[filter_occupancy]):
                try:
                    w_string = ":".join(
                        [self._all_ids[w_id] for w_id in self.hash_table[h]]
                    )
                except KeyError:
                    if allow_direct_bonds:
                        w_string = wire_key
                        ...
                        break
                try:
                    filtered_results[occupancy[i]].append(
                        (
                            w_string,
                            hash_index[filter_occupancy][i],
                            self.hashs[wire_key] == h,
                        )
                    )
                except:
        ...
        for occupancy in occupancy_dict:
            for wire, first_appearance_frame, occupancy_series in occupancy_dict[
                occupancy
            ]:
                nodes_in_wire = wire.split(":")
                for node in nodes_in_wire:
                    first_frame_dict[node] = first_appearance_frame
    ...
    def compute_average_water_per_wire(self, use_filtered=True):
        ...
        return {
            key: self.wire_lengths[key][self.wire_lengths[key] < _np.inf].mean()
            for key in results
        }
    ...
    def filter_minmal_bonds_path(self, start, goal, use_filtered=True):
        ...
        if goal in component:
            ...
            node_set = _nx.all_shortest_paths(
                weighted_component, start, goal, weight="weight"
            )
        else:
    ...
    def compute_all_shortest_paths_info(self, start, goal, use_filtered=True):
        ...
        for path in paths:
            key = (
                start
                + "-"
                + "-".join([node.split("-")[2] for node in path[1:-1]])
                + "-"
                + goal
            )
            val = _np.sum(
                [
                    avg[a + ":" + b] if a + ":" + b in avg else avg[b + ":" + a]
                    for a, b in _hf.pairwise(path)
                ]
            )
            res[key] = val
    ...
    def compute_wire_projection(self):
        ...
        for ts in self._universe.trajectory[self._trajectory_slice]:
            ...
            for rmsd_key in self.hashs:
                ...
                if rmsd_hash == frame_count or rmsd_hash == -1:
                    continue
                ...
                c_a, c_b, c_c = (
                    _np.repeat(
                        all_coordinates[rmsd_residues[0]].reshape((-1, 3)),
                        nb_water,
                        axis=0,
                    ),
                    _np.repeat(
                        all_coordinates[rmsd_residues[1]].reshape((-1, 3)),
                        nb_water,
                        axis=0,
                    ),
                    all_coordinates[rmsd_water].reshape((-1, 3)),
                )
    ...
    def draw_water_timeseries(
        self, resa, resb, scatter_size=0.5, filename=None, return_figure=False
    ):
        try:
        ...
        return_str = (
            ...
            + resb
            + " "
            + " ".join(timeseries.astype(_np.int).astype(_np.str))
        )
        if return_figure:
            _plt.close()
    ...
    def _save_average_and_occupancy_to_one_file(self, filename):
        ...
        for key in self.filtered_results:
            string += (
                key
                + " "
                + str(self.filtered_results[key].mean())
                + " "
                + str(avg_water[key])
                + "\n"
            )
