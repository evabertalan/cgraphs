...
class CompareTwo(ProteinGraphAnalyser):
    def __init__(
        ...
        if self.type_option == "pdb" and pdb1 and pdb2:
            ...
            ProteinGraphAnalyser.__init__(
                self,
                pdb_root_folder=self.compare_folder,
            ...
            ProteinGraphAnalyser.__init__(
                ...
                psf_files=[psf1, psf2],
                dcd_files=[dcd1, dcd2],
                sim_names=[name1, name2],
                plot_parameters=plot_parameters,
            )
            self.logger.info(
                "COMPARE SIMULATIONS " + self.name1 + " WITH " + self.name2
            )
        else:
            self.logger.warning("Required files are missing for the calculation")
    ...
    def construct_comparison_objects(self, occupancy=None):
        ...
        if self.type_option == "dcd":
            if self.occupancy:
                self.logger.info(
                    f"Filter graphs for {self.name1} and {self.name2} on occupancy {self.occupancy * 100}%."
                )
                graphs = []
    ...
    def plot_graph_comparison(
        ...
        color_propka=False,
        color_data=False,
        node_color_selection="protein",
        node_color_map="bwr",
        calcualte_distance=False,
    ):

        if len(self.graph_coord_objects.items()) != 2:
            self.logger.warning(
                f"There are {len(self.graph_coord_objects.items())} structures selected. Graph comparison is possible for exactly two structures."
            )
        else:
            self.logger.info(
                "Plot comparison "
                ...
                + " with "
                + self.name2
                + str(" colored by pKa" if color_propka else "")
                + str(" colored by user defined values" if color_data else "")
                + str(" with labels" if label_nodes else "")
            )
            if (
                "graph" in self.graph_coord_objects[self.name1].keys()
                and "graph" in self.graph_coord_objects[self.name2].keys()
            ):
                ...
                pos1 = self._get_node_positions(
                    self.graph_coord_objects[self.name1], pca=False
                )
                pos2 = self._get_node_positions(
                ...
                if calcualte_distance:
                    ...
                    bond_distances1 = self._get_edge_distance(
                        self.graph_coord_objects[self.name1],
                        self.name1,
                        self.compare_folder,
                        denumber_waters=True,
                    )
                    bond_distances2 = self._get_edge_distance(
                        self.graph_coord_objects[self.name2],
                        self.name2,
                        self.compare_folder,
                        denumber_waters=True,
                    )
                ...
                for i, pos in enumerate([pos1, pos2]):
                    for key, value in pos.items():
                        if key.split("-")[1] in _hf.water_types:
                            ...
                            if (
                                i == 1
                            ):  # get waters from pos2 since condensed water are initialized as pos1
                                for (
                                    conserved_key,
                                    conseved_vals,
                                ) in conserved_waters.items():
                                    if (value[0] - conseved_vals["pos"][0]) ** 2 + (
                                        value[1] - conseved_vals["pos"][1]
                                    ) ** 2 + (
                                        value[2] - conseved_vals["pos"][2]
                                    ) ** 2 <= float(
                                        1
                                    ) ** 2:
                                        # breakpoint()
                                        conserved_waters[conserved_key][
                                            "conserved"
                                        ] = True
                                        conserved_waters[conserved_key]["pair"] = key
                        if key not in all_pos.keys():
                            all_pos.update({key: value})
                ...
                fig, ax = _hf.create_plot(
                    title=f"{plot_name} graph comparison of {self.name1} with {self.name2} \n Selection: {self.selection[1:-16]}",
                    xlabel=xlabel,
                    ylabel=ylabel,
                    plot_parameters=self.plot_parameters,
                )

                fig_cons, ax_cons = _hf.create_plot(
                    title=f"{plot_name} conserved graph of {self.name1} and {self.name2} \n Selection: {self.selection[1:-16]}",
                    xlabel=xlabel,
                    ylabel=ylabel,
                    plot_parameters=self.plot_parameters,
                )
                ...
                if calcualte_distance:
                    fig_dist, ax_dist = _hf.create_plot(
                        title=f"{plot_name} conserved graph of {self.name1} and {self.name2} \n Selection: {self.selection[1:-16]}",
                        xlabel=xlabel,
                        ylabel=ylabel,
                        plot_parameters=self.plot_parameters,
                ...
                for e in graph1.edges:
                    e0 = (
                        e[0]
                        if e[0].split("-")[1] not in _hf.water_types
                        else f"1-{e[0].split('-')[1]}-{e[0].split('-')[2]}"
                    )
                    e1 = (
                        e[1]
                        if e[1].split("-")[1] not in _hf.water_types
                        else f"1-{e[1].split('-')[1]}-{e[1].split('-')[2]}"
                    )
                    if (
                        _hf.is_conserved_edge(
                            np.array([[e2[0], e2[1]] for e2 in graph2.edges]), e0, e1
                        )
                    ...
                    if e0 in node_pca_pos.keys() and e1 in node_pca_pos.keys():
                        ...
                        if e in conserved_edges:
                            ...
                            if calcualte_distance:
                                e0_chain_id, e0_res_name, e0_res_id = (
                                    _hf.get_node_name_pats(e[0])
                                )
                                e_0 = f"{e0_chain_id}-{e0_res_name}-{e0_res_id}"

                                e1_chain_id, e1_res_name, e1_res_id = (
                                    _hf.get_node_name_pats(e[1])
                                )
                ...
                if calcualte_distance:
                    ax_dist, fig_dist = self._plot_dist_plot(
                        dist_plot_data, ax_dist, fig_dist, node_color_map, label_edges
                    )
                    if label_nodes:
                        is_backbone = (
                            "_backbone" if self.include_backbone_sidechain else ""
                        )
                        _hf.write_text_file(
                            f"{self.compare_folder}H-bond_graph_distances_changes{is_backbone}.txt",
                            [
                                f"{edge} {round(distance_change,3)}\n"
                ...
                for e in graph2.edges:
                    e0 = (
                        e[0]
                        if e[0].split("-")[1] not in _hf.water_types
                        else "2-" + e[0].split("-")[1] + "-" + e[0].split("-")[2]
                    )
                    e1 = (
                        e[1]
                        if e[1].split("-")[1] not in _hf.water_types
                        else "2-" + e[1].split("-")[1] + "-" + e[1].split("-")[2]
                    )
                    conserved_pair = [v["pair"] for v in conserved_waters.values()]
                    if e0 in node_pca_pos.keys() and e1 in node_pca_pos.keys():
                        if (
                            not _hf.is_conserved_edge(
                                np.array([[e2[0], e2[1]] for e2 in graph1.edges]),
                                e0,
                                e1,
                            )
                            and e0 not in conserved_pair
                            and e1 not in conserved_pair
                        ):
                            edge_line = [node_pca_pos[e0], node_pca_pos[e1]]
                            x = [edge_line[0][0], edge_line[1][0]]
                            y = [edge_line[0][1], edge_line[1][1]]
                ...
                if color_propka or color_data:
                    color_info1 = self._custom_node_coloring(
                        self.graph_coord_objects[self.name1],
                        self.name1,
                        color_propka,
                        color_data,
                    ...
                    color_info2 = self._custom_node_coloring(
                        self.graph_coord_objects[self.name2],
                        self.name2,
                        color_propka,
                        color_data,
                    ...
                    if len(color_info1) and len(color_info2):
                        for key1, value1 in color_info1.items():
                            if key1 in color_info2.keys():
                                value_diff = float(color_info1[key1]) - float(
                                    color_info2[key1]
                                )

                                color_info.update({key1: value_diff})
                        value_colors, cmap, norm = _hf.get_color_map(
                            color_info, color_map=node_color_map, center=True
                        )
                    color_bar_label = (
                        "Amino acid data value" if color_data else "pKa value"
                    )
                    if label_nodes:
                        is_backbone = (
                            "_backbone" if self.include_backbone_sidechain else ""
                        ...
                        _hf.write_text_file(
                            f"{self.compare_folder}H-bond_graph_pKa_changes{is_backbone}.txt",
                            [
                                f"{edge} {round(distance_change,3)}\n"
                ...
                for n in graph1.nodes:
                    ...
                    if n in node_pca_pos.keys():
                        if n in graph2.nodes:
                            ...
                            color = color1
                        if (
                            n.split("-")[1] in _hf.water_types
                            and n not in conserved_waters.keys()
                        ):
                            ax.scatter(
                                node_pca_pos[n][0],
                                node_pca_pos[n][1],
                ...
                if label_nodes:
                    for n in all_pos.keys():
                        ...
                        if res_name in _hf.water_types:
                            if (
                                n not in conserved_waters.keys()
                                and n not in conserved_pair
                            ):
                                ax.annotate(
                                    f"W{res_id}",
                ...
                if color_info:
                    cbar = fig_cons.colorbar(
                        mpl.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax_cons
                    )
                    cbar.ax.tick_params(
                ...
                if self.graph_type == "hbond":
                    for form in self.plot_parameters["formats"]:
                        fig.savefig(
                            f"{self.compare_folder}compare_H-bond_graph_{self.name1}_with_{self.name2}{is_backbone}{is_label}.{form}",
                            format=form,
                            dpi=self.plot_parameters["plot_resolution"],
                        )
                        fig_cons.savefig(
                            f"{self.compare_folder}conserved_H-bond_graph_{self.name1}_with_{self.name2}{is_propka}{is_conservation}{is_backbone}{is_label}.{form}",
                            format=form,
                            dpi=self.plot_parameters["plot_resolution"],
                        )
                        if calcualte_distance:
                            fig_dist.savefig(
                                f"{self.compare_folder}conserved_H-bond_graph_{self.name1}_with_{self.name2}_distances{is_backbone}{is_label}.{form}",
                                format=form,
                                dpi=self.plot_parameters["plot_resolution"],
                            )
                    if is_label:
                        _hf.write_text_file(
                            ...
                            [
                                ...
                                + self.name1
                                + ": "
                                + str(len(graph1.nodes)),
                                "\nNumber of edges in "
                                + self.name1
                                + ": "
                                + str(len(graph1.edges)),
                                "\n",
                                "\nNumber of nodes in "
                                + self.name2
                                + ": "
                                + str(len(graph2.nodes)),
                                "\nNumber of edges in "
                                + self.name2
                                + ": "
                                + str(len(graph2.edges)),
                                "\n",
                                "\nNumber of nodes in both structures: "
                                + str(len(conserved_nodes)),
                                "\nNumber of edges in both structures: "
                                + str(len(conserved_edges)),
                                "\n",
                                "\nList of nodes in both structures: "
                                + str(conserved_nodes),
                                "\n",
                                "\nList of edges in both structures: "
                                + str(conserved_edges),
                            ],
                        )
                    ...
                    for form in self.plot_parameters["formats"]:
                        fig.savefig(
                            f"{self.compare_folder}compare{waters}{occ}_graph_{self.name1}_with_{self.name2}{is_label}.{form}",
                            format=form,
                            dpi=self.plot_parameters["plot_resolution"],
                        )
                        fig_cons.savefig(
                            f"{self.compare_folder}conserved{waters}{occ}_graph_{self.name1}_with_{self.name2}{is_propka}{is_conservation}{is_label}.{form}",
                            format=form,
                            dpi=self.plot_parameters["plot_resolution"],
                        )
                    if is_label:
                        _hf.write_text_file(
                            ...
                            + "_info.txt",
                            [
                                "Water wire graph comparison of "
                                + self.name1
                                + " with "
                                + self.name2,
                                "\nSelection string: " + str(self.selection[0:-15]),
                ...
                self.logger.warning(
                    "No "
                    + self.graph_type
                    + " graph for both structures. Comparison can not be performed."
                )

    def _custom_node_coloring(
        self, objects, name, color_propka, color_data, node_color_selection
    ):
        color_info = {}
        if color_propka and color_data:
            ...
            struct_object = (
                objects["structure"]
                if not hasattr(self, "occupancy")
                else objects["mda"]
            )
            selected_nodes = struct_object.select_atoms(str(node_color_selection))
            try:
                color_info = _hf.read_propka_file(
                    f"{self.target_folder}/{name}.pka", selected_nodes
                )
            except:
                self.logger.info(
                    f"{name}.pka not found. To color residues by pKa values, place the propka file in the PDB folder, next to the PDB file."
                )
            try:
                len(color_info)
            except:
            ...
            color_info = _hf.read_color_data_file(
                name, self.target_folder, selected_nodes
            )
            try:
                len(color_info)
            except:
                self.logger.error(
                    f"No {name}_data .txt file was found in {self.target_folder} or content is invalid. To enable coloring by data values please add a corresponding file."
                )
        return color_info
    ...
    def _plot_dist_plot(self, dist_plot_data, ax, fig, color_map, label_edges):
        ...
        scaled_values = (distances - distances.min()) / (
            distances.max() - distances.min()
        )
        value_colors = [
            (
                self.plot_parameters["graph_color"]
                if round(distances[i], 1) == 0
                else cmap(scaled_values[i])
            )
        ...
        for i, point in enumerate(dist_plot_data):
            ...
            ax.plot(
                ...
                markersize=self.plot_parameters["node_size"] * 0.01,
                markerfacecolor=self.plot_parameters["graph_color"],
                markeredgecolor=value_colors[i],
                path_effects=[
                    pe.Stroke(
                        linewidth=2, foreground=self.plot_parameters["graph_color"]
                    ),
                    pe.Normal(),
                ],
            )
            ...
            if label_edges:
                ax.annotate(
                    round(point[2], 1),
                    (x[0] + (x[1] - x[0]) / 2, y[0] + (y[1] - 1.0 - y[0]) / 2),
                    color="blue",
