                ...
                if color_propka and color_data:
                    ...
                    try:
                        color_info = _hf.read_propka_file(
                            f"{self.pdb_root_folder}/{name}.pka", selected_nodes
                        )
                    except:
                        self.logger.info(
                    ...
                    try:
                        ...
                        value_colors, cmap, norm = _hf.get_color_map(
                            color_info, color_map=node_color_map, normalize=[0, 15]
                        )
                        self.logger.info(f"Color {name} by pKa values{lab}.")
                        color_bar_label = "pKa value"
                    except:
                        self.logger.info(
                            f"{name}.pka does not contain the selected residues. Please update the Residues to color!"
                        )
