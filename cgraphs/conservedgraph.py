...
class ConservedGraph(ProteinGraphAnalyser):
    def __init__(
        ...
        if type_option == "pdb":
            ...
            raise ValueError('Given type_option should be "pdb" or "dcd"')
        ...
        if self.graph_type == "water_wire":
            ...
            for objects in self.graph_coord_objects.values():
                if "graph" in objects.keys():
                    graph = objects["graph"]
                    waters = {}
                    for res in objects["structure"].select_atoms(
                        "(protein and name CA) or" + _hf.water_def
                    ):
                        if res.resname in _hf.water_types:
                            waters[res.resid] = res.position
                    #             for graph in self.graphs:
                    # here select conserved water by the superimposed ones
                    #                 if useEstimatedConservedWaters:
                    #                     check whether water coordinates belong to any cluster thatn give name
                    for node in graph.nodes:
