...
def compare_view(self):
    ...
    psf1_field = tk.Entry(dcd_compare_tab, state='disabled', xscrollcommand=s3.set)
    psf1_field.grid(row=2, column=1, sticky="EW")
    s3.configure(command=psf1_field.xview)

    tk.Button(dcd_compare_tab, text='DCDs 1', command=lambda:self._select_dcd1_compare(dcd1_field)).grid(row=4, column=0, sticky="EW")
    s4 = self._add_horisontal_scroll(dcd_compare_tab, row=5, column=1)
    dcd1_field = tk.Entry(dcd_compare_tab, state='disabled', xscrollcommand=s4.set)
    dcd1_field.grid(row=4, column=1, sticky="EW")
    s4.configure(command=dcd1_field.xview)

    tk.Label(dcd_compare_tab, text='Name 1: ', anchor='w').grid(row=6, column=0, sticky='W')
    # self.sim_name1 = tk.StringVar(value='sim1')
    self.compare_dcd1_name = tk.Entry(dcd_compare_tab)
    ...
    color_dcd_field1 = tk.Label(dcd_compare_tab, width=2, bg=self.color_dcd1, anchor="w")
    ...
    tk.Button(dcd_compare_tab, text='PSF 2', command=lambda:self._select_psf2_compare(psf2_field)).grid(row=7, column=0, sticky="EW")
    s5 = self._add_horisontal_scroll(dcd_compare_tab, row=8, column=1)
    psf2_field = tk.Entry(dcd_compare_tab, state='disabled', xscrollcommand=s5.set)
    psf2_field.grid(row=7, column=1, sticky="EW")
    s5.configure(command=psf2_field.xview)

    tk.Button(dcd_compare_tab, text='DCDs 2', command=lambda:self._select_dcd2_compare(dcd2_field)).grid(row=9, column=0, sticky="EW")
    s6 = self._add_horisontal_scroll(dcd_compare_tab, row=10, column=1)
    dcd2_field = tk.Entry(dcd_compare_tab, state='disabled', xscrollcommand=s6.set)
    dcd2_field.grid(row=9, column=1, sticky="EW")
    s6.configure(command=dcd2_field.xview)

    tk.Label(dcd_compare_tab, text='Name 2: ', anchor='w').grid(row=11, column=0, sticky='W')
    # self.sim_name1 = tk.StringVar(value='sim1')
    self.compare_dcd2_name = tk.Entry(dcd_compare_tab)
    self.compare_dcd2_name.insert(0, 'sim2') # remove when test resolved
    self.compare_dcd2_name.grid(row=11, column=1, sticky="EW")
    l1 = tk.Label(dcd_compare_tab, text='Give a unique name to your simulation. Calculations with the same name are overwritten.', anchor='w')
    l1.grid(row=12, column=1, sticky='EW')
    l1.config(font=("Helvetica", 11))
    self.color_dcd2 = '#21c25f'
    color_dcd_field2 = tk.Label(dcd_compare_tab, width=2, bg=self.color_dcd2, anchor="w")
    ...
    tk.Button(dcd_compare_tab, text='Construct graph', command=lambda:self._construct_compare_graphs(psf1=self.psf_1, psf2=self.psf_2, dcd1=self.dcd_1, dcd2=self.dcd_2)).grid(self._create_big_button_grid(13, column=1))

    # -------------------water_wire_frame -----------------------

    water_wire_frame_dcd = ttk.LabelFrame(dcd_compare_tab, text='Water wire network')
    water_wire_frame_dcd.grid(self._crate_frame_grid(14))
    ...
    tk.Button(water_wire_frame_dcd, text='Compare water wire network', command=lambda:self._plot_dcd_comparison(color1=self.color_dcd1, color2=self.color_dcd2), width=self.button_width).grid(self._create_big_button_grid(2))
